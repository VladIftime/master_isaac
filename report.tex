...
Both environments were trained and validated on a high-performance computing cluster. The throwing policy, trained across 4,096 parallel environments, reduced its mean best landing distance from 0.28~m to 0.13~m and increased its per-attempt basket landing rate from 7.5\% to 43\% between 20 million and 147 million training steps. The table-tennis policy reached an 18\% paddle contact rate. Training benchmarks show 7.1$\times$ scaling from 128 to 1,024 parallel simulation instances. The transition from a CPU-bound simulator to a GPU-accelerated platform was instrumental in achieving these results: the orders-of-magnitude increase in simulation output enabled rapid hyperparameter exploration and substantially faster policy convergence. The throwing pipeline reproduces a prior formulation that achieved 94\% accuracy in a CPU-bound Gazebo setting with a single simulation instance; the GPU-accelerated platform generates nearly 3,000 times more environment interactions in comparable wall-clock time.
\end{abstract}
...
Both environments share a common dual-arm platform: the robot stands on a 0.6~m base with a 2.0$\times$1.7~m work table in front, and Cartesian-space target commands are translated into joint-space motions via an inverse-kinematics solver supporting four backends. By default, only one arm is active while the other serves as a static collision obstacle. This design isolates the learning variables of each task from the complexity of full bi-manual coordination. The shared robot model and solver infrastructure directly support future dual-arm tasks.
...
\end{subfigure}
\caption{The throwing environment. The robot stands on a 0.6~m base with a work table and a randomised target basket. Left: start of throw, after attachment variant. Centre: throw, using dynamic friction variant. Right: drink lands in the basket with touch sensor at the bottom.}
\label{fig:throw_env}
\end{figure}
...
\begin{figure}[t!]
\centering
\includegraphics[width=0.48\textwidth]{report_pics/table_tennis_train.png}
\caption{Training progression for the throwing environment, showing multiple enviroments training in parallel.}
...
A prior throwing study using a CPU-bound Gazebo simulator trained a policy for 50,000 steps with a single simulation instance, achieving 94\% accuracy~\cite{kasaei2023learning}. The GPU-accelerated implementation presented here runs 4,096 parallel environments simultaneously and reaches 147 million training steps in approximately 27 wall-clock hours across multiple HPC job submissions, nearly 3,000 times the step count of the prior work. Where the Gazebo formulation was limited to one sequential environment interaction per physics step, the GPU platform produces 4,096 transitions per outer iteration, fundamentally changing the volume of experience available to the learning algorithm. Figures~\ref{fig:throw_train} and~\ref{fig:tt_train} in Section~\ref{sec:environments} show the training progression for both environments, illustrating how the increased simulation throughput translates directly to faster policy convergence.
...
\begin{figure}[t!]
\centering
\includegraphics[width=0.48\textwidth]{report_pics/throw_train.png}
\caption{Throwing enviroment training  progress , showing multiple enviroments training in parallel.}
\label{fig:tt_valid}
\end{figure}

A policy trained for 130,000 iterations was evaluated deterministically against 100 serve episodes. Each episode was classified by its terminal outcome. \emph{Immediate floor drop} indicates the ball fell below table height ($z < 0.65$~m) without the paddle ever approaching within the contact detection threshold. \emph{Hits own table} indicates the ball bounced a second time on the robot's own table half, ending the rally in a fault. \emph{Paddle contact made} counts episodes where the ball entered a 0.06~m virtual detection threshold around the paddle surface point at any time during the rally; once triggered, the contact flag is latched for the remainder of the episode. \emph{Reaches opponent zone} requires paddle contact followed by the ball landing on the opponent's table half within the regulation zone bounds, which constitutes a successful return. Mean reward is the arithmetic mean of total per-episode reward, composed of weighted terms for paddle contact ($+1.0$), opponent-table success ($+5.0$), own-table fault ($-2.0$), floor drop ($-3.5$), return velocity ($+0.5$), and forward ball-position shaping ($+2.0$). Table~\ref{tab:tt_results} summarises the outcomes.
...
The 18\% contact rate indicates the policy has begun to approach the incoming ball, with a mean peak contact score of 0.37 across contacted episodes. The contact score is a continuous 0--1 value that increases linearly from 0 at the 0.06~m detection threshold to 1 at zero distance; a peak score of 0.37 corresponds to a closest approach of approximately 0.038~m from the paddle surface. Most episodes (82\%) ended with the ball immediately hitting the floor without any paddle interaction. At this stage of training, a non-zero contact rate confirms the learning signal is functional; further training is expected to improve return accuracy as the velocity and position-based reward components strengthen.

\subsection{Object Throwing}

\begin{figure}[t!]
\centering
\includegraphics[width=0.5\textwidth]{report_pics/comparison_birdseye.png}
\caption{Throwing validation: bird's-eye view of mean landing positions per target for the 20M-step and 147M-step checkpoints. Targets shown as squares; mean landings as circles and diamonds. Dashed lines connect each target to its mean landing position.}
\label{fig:throw_birdseye}
\end{figure}

\begin{figure}[t!]
\centering
\includegraphics[width=0.85\textwidth]{report_pics/comparison_distances.png}
\caption{Throwing validation: best and mean Euclidean distance to target per test, comparing the 20M-step and 147M-step checkpoints across 10 targets with 20 attempts each.}
\label{fig:throw_distances}
\end{figure}

The throwing policy was trained using 4,096 parallel environments and evaluated against 10 predefined target positions with 20 attempts each (Figures~\ref{fig:throw_birdseye} and~\ref{fig:throw_distances}). Table~\ref{tab:throw_progression} compares the policy at two checkpoints from the same training run.

Euclidean distance measures the straight-line distance from the object's final resting position to the basket centre. \emph{Targets reached} counts the number of targets for which at least one of the 20 attempts landed inside the basket's axis-aligned bounding box (AABB), an overlap check in X, Y, and Z against the scaled basket dimensions. \emph{Mean best distance} is the average, across all 10 targets, of the minimum distance achieved in 20 attempts. \emph{Mean distance} averages all 200 individual throws. The \emph{per-attempt basket landing rate} is the fraction of all 200 throws that produced an AABB hit.
...
\begin{tabular}{@{} l c c @{}}
\toprule
\textbf{Metric} & \textbf{20M Steps} & \textbf{147M Steps} \\
\midrule
Targets reached ($\geq$1 AABB hit) & 6 / 10 (60\%)  & 8 / 10 (80\%) \\
Mean best distance                  & 0.28~m         & 0.13~m        \\
Mean distance                       & 0.99~m         & 0.46~m        \\
Per-attempt basket landing rate     & 7.5\%          & 43.0\%        \\
\bottomrule
\end{tabular}
\end{table}

At 20 million steps the policy already differentiates between target positions, reaching 6 of 10 targets at least once. By 147 million steps, completed in approximately 27 wall-clock hours across multiple HPC job submissions, mean best distance halved to 0.13~m and per-attempt basket landing rate increased nearly sixfold to 43\%. The prior Gazebo formulation achieved 94\% accuracy after 50,000 steps with a single simulation instance~\cite{kasaei2023learning}; the GPU platform's 4,096-fold parallelism trades per-step sample efficiency for a massive increase in total experience, generating nearly 3,000 times more environment interactions in comparable wall-clock time.
...
The empirical results confirm the approach is viable. The throwing policy, trained across 4,096 parallel environments, reduced mean best distance from 0.28~m to 0.13~m and increased per-attempt basket landing rate from 7.5\% to 43\% over 147 million training steps, a nearly 3,000-fold increase in experience over the prior single-instance Gazebo formulation. The table-tennis policy reached an 18\% paddle contact rate. Training benchmarks show 7.1$\times$ scaling and a 1.41$\times$ speedup over the standard environment path.
