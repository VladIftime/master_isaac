...
**Last updated**: 2026-08-13 18:40 CEST (campaign scoped to thesis-only pbrsA/pbrsB 1024 2-seed CI; validation-speed: max_pushes 10/max_tries 3, gpushort partition, gym 8G; 80 validation jobs submitted under campaign val_fast; queue: 23 gpushort jobs, 7 pending + 3 running)
...
| 13 | **PBRS Model I** — Model G + Bob time penalty R_B += -gamma_sp*t_B | `train_i_tasp_dpose_bobpen.py` |
...
| 2026-06-29 | **PBRS Model I implemented** — Fork of Model G (`train_i_tasp_dpose_bobpen.py`, 1508 lines). Adds Bob time penalty `R_B += -gamma_sp * t_B` at Bob phase end, completing the full symmetric Sukhbaatar et al. 2018 reward structure. Alice reward unchanged (`R_A = gamma_sp * max(0, t_B - t_A)`). The two time-based rewards together form a near-zero-sum competitive game: Alice gains from proposing goals Bob takes many pushes to solve; Bob is penalized for inefficient solving. Penalty applied on all three Bob phase-end paths: normal `handle_bob_phase_end`, early `bob_achieved_completion`, and catastrophe. Args: `--bob_time_penalty` (default on), `--no_bob_time_penalty` (disable for Model G behavior), `--bob_time_penalty_scale` (default 0.5). Logged to TensorBoard as `Reward/Bob/TimePenalty`. HPC script: `hpc/train_i_tasp_dpose_bobpen.slurm` (fork of Model G slurm, `EXP_NAME=hpc_pbrs_tasp_dpose_bp_${NUM_ENVS}env`). |
| 2026-06-19 | **Future models planned** — Model J: H + Bob time penalty (disc variant). Model K/L: G/H + ABC enabled (beta=0.5) to test whether time-based Alice + ABC produces stronger curriculum than either alone. |
...
| 2026-06-29 | **SAC+HER Fix S3 (cloudpickle/USD stage crash on checkpoint save)** — `LatestCheckpointCallback._on_step()` called `model.save()` which triggers SB3's `data_to_json()` → `cloudpickle.dumps(gym.spaces.Dict)`. Cloudpickle chased the observation space's object graph into Isaac Lab's `pxr.Usd.Stage` (unpicklable). Fixed: wrapped `model.save()` in `try/except Exception` in callback, with `include=["replay_buffer"]` for consistency with SIGTERM handler. Training continues past the failed save. |
...
| 2026-07-12 | **Validation-path fixes (§14.10, V1–V9)** — (V1) added `DISC_TESTS` to `validation_configs.py` — the 30 T-block scenes mirrored onto the disc (position-only, `goal_yaw=0`, `object_type=disc`) + a `--scene-set {all,disc,tblock}` flag on `validate_push_asp.py`, so the ABC-on disc positive control is scored on disc scenes it actually trained on (was silently validated on T-block scenes → void). (V2) `model_best.pt` in `train_e`/`train_g` now gated on a **combined-gate SR** (`pos<0.05m ∧ rot<0.2rad`) matching the validation gate, not the looser training d_pose≤0.055 (`Metrics/Bob/CombinedSR` logged). (V4) templates no longer treat exit 137/143 as preemption — only the USR1 time-limit path resubmits, so an **OOM SIGKILL now trips the brake** instead of silently resubmitting ≤10× (protects the 2048-env/48G jobs). (V5/V6) `submit_all.sh` reconciles validators too (skip if the output CSV exists; skip runs with no usable checkpoint), and `validate.slurm` gained `--allow-latest` → validates `latest_checkpoint.pt` when `model_best.pt` is absent, writing to a separate `validation_latest/` subfolder (distinct filenames, excluded from collation). (V8) `collate_seeds.py` regex now accepts an optional trailing `_<tag>` (so `_noge/_abc/_nohist/_pc` ablations aggregate as their own rows instead of being silently skipped) and a gym `_e_p_s` naming variant. (V9) gym is treated as quantitative: new `hpc/arrays/validate_gym.slurm` (runs `validate_pusht_gym.py`, auto-detects bob/agent), `gen_params.py` emits `phase5_gym_validate`, and the collator ingests gym CSVs. (V3/V7 confirmed non-issues: the ASP validator already scores the combined gate for T-block and position-only for disc; noge is auto-detected from checkpoint weights.) |
| 2026-07-12 | **Disc orientation-zeroing subspace + T-block→disc cross-eval (§14.11)** — shared `reward_pbrs.dpose_and_zero_yaw()` transform (writes `[d_pose,bearing]` tail; zeros obj/goal yaw iff `char_length==0`) used identically by `wrapper_push_asp._get_push_obs`/`construct_bob_observation` and the validator's disc `_build_obs` (which previously emitted `[pos_dist,rot_dist]` — a latent Model F/H obs mismatch, now fixed). Makes the disc a strict orientation-free subspace of the T-block obs. New `phase4_xeval` (validation-only): E/G `model_best` evaluated on disc scenes at `--char-length 0.0 --scene-set disc`; disc-scene evals write an `_sdisc`-tagged CSV so a T-block model's cross-eval never collides with its native validation. Cross-object load verified safe (`ValidateEnvCfg` always has `disc_object`; GoalEncoder/`pi_obj_dim` auto-detected; char_length/scene-set are eval-time flags). Model F/H + `phase4_pc` retrain under the updated wrapper; E/G unaffected. |
| 2026-07-12 | **Subspace review follow-ups** — (1) `char_length==0.0` exact float-equality replaced by `< 1e-6` in `dpose_and_zero_yaw` + the wrapper subspace log (a future `1e-9` could otherwise silently disable yaw-zeroing while d_pose stayed position-only → silent train/eval mismatch). (2) **Seed wiring added to `train_f`/`train_h`/`train_i`** (`torch/cuda/np/random` + `env_cfg.seed`) — they were unseeded, so the multi-seed `phase4_pc`/`phase3b` runs would have been undifferentiated. (2b) **V2 combined-gate `model_best`** applied to `train_i` (T-block; `Metrics/Bob/CombinedSR`); `train_h` stays position-only (disc). (2c) **Models H and I added to the campaign** (`gen_params.py`): `taspH` (disc, validated `--scene-set disc`) and `taspI` (T-block Bob-penalty) at 5 seeds each in new `phase3b`, so they're trained + validated rather than orphaned. (3) `_get_alice_obs` defensively zeros the object-yaw slot in the disc subspace so Alice matches Bob regardless of caller (idempotent with the upstream obs zeroing). (4) `_get_goal_euler` defensively zeros goal-yaw in the disc subspace — the single chokepoint for all reward/metric consumers (`_rot_distance_rad`, `_yaw_distance_rad`, `compute_dpose`), so no random disc goal-yaw can leak into `rot_err` or the dense reward. |
| 2026-07-12 | **Preemption-safe exit handling (§14.9 revision) + obs smoke flag** — Confirmed all Hábrók GPU partitions are `PreemptType=preempt/qos`, `PreemptMode=REQUEUE`: a preemption exits 137/143 and SLURM **auto-requeues the same job**. The previous "137/143 → self-resubmit" would then run a duplicate alongside SLURM's requeue → concurrent writes to the same `latest_checkpoint.pt` (corruption). Fix (option B): the 3 array templates now carry `#SBATCH --requeue` and, on a 137/143 exit, **do nothing** — no self-resubmit (avoids the duplicate), no brake (a preemption must not kill the campaign), no `.done`; SLURM requeues and our resume logic continues. Time-limit still self-resubmits via the USR1 path; genuine crashes / CUDA-VRAM OOM (non-137 exit) still trip the brake; host-RAM OOM (rare, mem right-sized) is left to SLURM's requeue. Also added a `--smoke_obs N` diagnostic flag to `train_e`/`train_f`: runs N obs-only steps (random object + goal yaw), prints `obj_yaw`/`goal_yaw`/`d_pose`/`alice_obj_yaw`, asserts the yaw slots are 0 in the disc subspace (`char_length<1e-6`) and exits before training — cheap runtime confirmation of the `_get_push_obs`/`_get_obj_euler`/`_get_goal_euler`/`_get_alice_obs`/`dpose_and_zero_yaw` path. **Confirmed (no change):** `model_best` saves are gated solely on `bob_combined_sr` in `train_e/g/i`; `phase3b_validate` scores `taspI` on the T-block combined gate and `taspH` on the disc position-only gate. |
| 2026-07-12 | **REQUEUE-cluster checkpoint durability + integrity (R1–R4)** — Hardening for the preempt/qos REQUEUE cluster: **(R1a atomic saves)** `PPO.save()` now writes `<path>.tmp` in the **same node-local dir** then `os.replace` (atomic; a preemption mid-save can't truncate `latest_checkpoint.pt`); it also embeds the `iteration` (protects a/b/e/f/g/i/push; PPOABC inherits it). **(R1b periodic flush)** each training template runs a host-side background `rsync $LOCAL_RUNS→$RESULTS_ROOT` every `FLUSH_SECS` (600s), killed after `wait` and in `cleanup()`, so a preemption/OOM that cuts `cleanup()` short still leaves a recent durable checkpoint (a Python-side flush was infeasible — `$RESULTS_ROOT` isn't bind-mounted in the container). **(R1c group-consistency guard)** a self-play checkpoint is a multi-file set (alice+bob+`latest_iter.txt`+episode_manager/abc_buffer); per-file atomicity doesn't make the *set* a coherent snapshot, so a partial flush could pair new-bob with old-alice and silently resume co-evolved adversaries from different points. Fix: `latest_checkpoint` saves embed `iteration=bob_updates` (both agents), `latest_iter.txt` written last, and on resume `train_e/g/f/i` peek each checkpoint's embedded iteration and, if `alice≠bob≠latest_iter`, **refuse the mismatched set and start that run fresh** (loud `[Resume][GUARD]` warning; campaign not killed — reconcile re-runs it). **(R2 requeue cap)** `SLURM_RESTART_COUNT ≥ 25` → `scontrol update JobId … Requeue=0` + FAIL-log + `exit 0` (stops only the offending task; no brake — legitimate preemption thrashing is tolerated, and the count is per-23h-segment since USR1 resubmits spawn a fresh jobid; reconcile self-heals a false stop). **(R4 smoke fix)** `--smoke_obs` now populates `goal_states` via `store_goal_state` with a random non-zero yaw (+ `current_phase=1`, `goal_valid=True`) and **hard-fails** if `goal_states` can't be populated — the previous `try/except: pass` made the goal-yaw check a silent no-op against a trivially-zero goal. **(R3)** no code change beyond R1a; run `scontrol show config \| grep -iE 'GraceTime\|KillWait'` on the cluster to size the preemption budget (R1b's periodic flush makes the in-GraceTime SIGTERM save non-critical). |
| 2026-07-12 | **G2 (latest_iter.txt demoted) + resume-state restore fixes** — `latest_iter.txt` is written/copied last, so a torn/missing txt was the most likely partial-flush state — yet the resume path treated it as authoritative and, worse, the template *only* resumed if the txt was readable (an intact alice+bob pair with a torn txt caused a full FRESH restart, losing progress). **G2-C1:** the group-consistency guard now tests the embedded iterations only (`alice_iter != bob_iter` → refuse), dropping `latest_iter.txt` from the equality — a torn txt alone no longer discards a consistent pair. **G2-C2:** after a guard-passed load, the resume iteration is taken from `bob_ppo.loaded_iteration` (the embedded, authoritative value), overriding the SLURM-passed `--resume_iteration`. **G2-C3:** `train_self_play.slurm` now passes `--chkpt_alice/--chkpt_bob` whenever both checkpoints exist (even if `latest_iter.txt` is unreadable), with `--resume_iteration ${ITER_NUM:-0}` as a fallback Python overrides — so a torn txt is non-fatal end-to-end. (C3 scoped to the self-play template, which pairs with C2 in e/g/f/i; the single-agent/gym templates keep their txt gate — no mismatch risk there, and a torn txt merely triggers a safe fresh re-run via reconcile.) **Adjacent resume-state fixes (train_e/g/f/i):** (1) the episode_manager/train_state resume paths used a stale `args.chkpt_bob.replace("model_", …)` that is a **no-op** on `latest_checkpoint.pt` → they silently restored nothing (guarded `load_state_dict` → no crash, but curriculum/coeff state reset); now derived as `dirname(chkpt_bob)/episode_manager_latest.pt` and `/train_state_latest.pt`. (2) **`best_bob_success_rate` was never persisted** and re-init to `-1.0` at every start, so `model_best.pt` was overwritten by the first post-restart checkpoint — meaning on the REQUEUE cluster (and even via the 23h resubmit chain) validation could load a non-global-best checkpoint. It is now saved in `train_state_latest.pt` and restored on resume, so `model_best` reflects the global best across requeues/resubmits. |
| 2026-07-12 | **G2 extended to single-agent + gym; gym-C removed from push_nsteps sweep** — Applied the embedded-iteration authority to the remaining trainers: `train_a`/`train_b` (single-agent) and `train_a_gym`/`train_b_gym` (gym single-agent) now override `--resume_iteration` from `agent.loaded_iteration` after load and tag `latest_checkpoint` saves with `iteration=iteration`; `train_c_gym` (gym ASP) gets the same `bob_ppo.loaded_iteration` override + an `alice≠bob` group-consistency guard + `iteration=bob_updates` on saves. Templates relaxed so a torn/missing `latest_iter.txt` no longer forces a fresh restart: `train_single_agent.slurm` resumes whenever `agent/latest_checkpoint.pt` exists (baseline `model_<iter>.pt` filename-iter branch unchanged), and `train_gym.slurm` now resumes both single-agent (`agent/`) and ASP (`bob/`+`alice/`) with `--resume_iteration ${ITER_NUM:-0}`. `train_push` unchanged (its `model_<iter>.pt` filename is already authoritative). **Also fixed a latent gym bug:** `train_c_gym` has no `--push_nsteps` (its rollout is `alice_pushes+bob_pushes`), so the gym template's `--push_nsteps` would have been an unrecognised arg → crash → brake. Gym-C is therefore removed from the E3 `phase5_gym` push_nsteps sweep (which is a single-agent rollout-length study); `phase5_gym` is now gymA/gymB × {64,256} × {15,45} = 8 jobs. The gym-ASP resume plumbing is kept (harmless) for a future dedicated gym-C run. |
| 2026-07-12 | **Thesis-plan implementation (A1–A6, B) — ASP-failure campaign refocus** — **(A1, sampling)** `validate_push_asp.py:599` replaced the argmax action-selection with a stochastic `MultiCategorical(raw_logits).sample()`; the single-agent validator already sampled via `act_with_hidden→dist.sample()` (confirmed). Both validators now produce genuine best-of-20 draws; single-agent and ASP are compared under one protocol. **(A2, canonical ABC-on)** `gen_params.py` rewritten for the thesis_plan.md matrix (§2.1): `--no_abc` removed from `SELFPLAY_EXTRA`/`G_EXTRA`/`discF`/`taspH`/`taspI` — headline self-play is ABC-on (β=0.5 canonical). The `no_abc` arm is now an ablation only (phase5); the discF ABC-off counterfactual is `phase6_abcpc`. **(A3, `--no_ge` on G)** ported from train_e to `train_g_tasp_dpose.py` (arg definition + bob-cfg conditional), enabling both E and G in the no-GE ablation arm. **(A6, gradient-update count)** `Diagnostics/GradientUpdates` logged in `train_a` (iteration), `train_e` and `train_g` (bob_updates) — the scale-sweep batch-vs-update confound is now quantifiable from training logs. **(B, gym-C push_nsteps)** `train_c_gym_pbrs_asp.py` gained `--push_nsteps` (accepted for gym-template compatibility; ASP rollout is alice+bob, not nsteps-based), re-enabling gym-C in the `phase8_gym` cross-env phase. Final matrix: 94 Isaac training jobs + 10 gym = 104 total. Phases: phase1_anchor (baseline+PBRS @4 sizes×3 seeds=24), phase2_ci (5 models @528×5 seeds=25), phase3_disc (discF/taspH @528×5 seeds=10), phase4_scale (E,G @{256,2048}×3 seeds=12), phase5_ablate (E,G no-ABC/no-GE @528×3 seeds=12), phase6_abcpc (discF ABC-on+off @528×3 seeds=6), phase7_bobpen (taspI @528×5 seeds=5), phase8_gym (A/B/C @2 sizes=10). |
| 2026-07-12 | **`model_best` persistence for single-agent + gym (closes the last overwrite hole)** — Self-play (e/g/f/i) already persist `best_bob_success_rate` in `train_state_latest.pt` (Fix#2); the single-agent and gym trainers did not, so on the REQUEUE cluster / 23h resubmit chain their `best_*` re-init to `-1.0` and `model_best.pt` was overwritten by the first post-restart checkpoint → validation could load a non-global-best model. Fixed in `train_a`, `train_b`, `train_push`, `train_a_gym`, `train_b_gym`, `train_c_gym`: each now (a) on a model-best save also writes the best value to `best_sr.txt` in the model-best dir (atomic `.tmp`→`os.replace`), and (b) on resume (`--chkpt`/`--chkpt_bob` set) restores `best_success_rate`/`best_sr`/`best_bob_sr` from `best_sr.txt`, so `model_best` reflects the global best across requeues/resubmits. The file rides the same node-local↔scratch rsync as the checkpoints. `train_push` (baseline, `model_<iter>.pt` scheme) included since its `model_best.pt` is what validation loads. All models — self-play, single-agent, gym, baseline — now select `model_best` on a restart-durable global criterion. |
| 2026-07-12 | **H2 (train_h episode_manager/train_state path no-op fix)** — `train_h`'s resume path constructed auxiliary state paths via `args.chkpt_bob.replace("model_", "episode_manager_")`, which is a **no-op** when `args.chkpt_bob="latest_checkpoint.pt"` — it keeps the literal string unchanged, silently loading nothing (guarded `if os.path.isfile` → no crash, no recovered state). Derivation changed to `os.path.join(os.path.dirname(args.chkpt_bob), "episode_manager_latest.pt")` and `"train_state_latest.pt"`, matching the other script-periodic/emergency saves. |
| 2026-07-12 | **H3 (train_h group-consistency guard + loaded_iteration authority)** — `train_h` was missing the per-file atomicity guard: a partial rsync flush could pair a new-bob checkpoint with an old-alice checkpoint and silently resume co-evolved adversaries from different training points. Added the `_peek_iter` guard that compares embedded iterations and refuses mismatched pairs (loud warning, fresh restart). Also added the `bob_ppo.loaded_iteration` authoritative override (`args.resume_iteration = int(bob_ppo.loaded_iteration)`) so the resume iteration comes from the checkpoint-embedded value, not the advisory `latest_iter.txt`. |
| 2026-07-12 | **H4 (ppo.load dim mismatch loud warning)** — `PPO.load()` uses `load_state_dict(strict=False)`, silently skipping layers whose shapes don't match between the checkpoint and the current model — a wrong-architecture checkpoint (T-block→disc, E→F, alice→bob) loads without error, leaving mismatched layers with random weights. Added `_warn_shape_mismatch()`: compares keys/shapes before every load and prints a loud `[PPO LOAD] Shape mismatch:` warning to stdout/slurm log with the specific missing/unexpected/shape-diff keys. The `strict=False` load still proceeds (backward compatible), but any architecture mismatch is now impossible to miss in the logs. |
| 2026-07-12 | **V10 (validation --argmax flag for sampling-vs-deterministic comparison)** — Added `--argmax` flag to both validators. `MultiCategorical` gained a `.mode()` method (`logits.argmax(dim=-1)`, deterministic). `validate_push.py` passes `deterministic=args.argmax` to `act_with_hidden`; `validate_push_asp.py` uses `_dist.mode()` when `--argmax` is true. Both validators default to stochastic sampling (best-of-20 protocol), but a parallel `--argmax` run enables verifying the claim that sampling and argmax produce the same result in the deterministic env. |
| 2026-07-12 | **G3 (gradient-update count — full coverage)** — `Diagnostics/GradientUpdates` was logged only in `train_a`, `train_e`, and `train_g`. Added to the 8 missing scripts (`train_push`, `train_b_pbrs_curriculum`, `train_f_pbrs_asp_disc`, `train_h_tasp_disc`, `train_i_tasp_dpose_bobpen`, `train_a_gym_pbrs_simple`, `train_b_gym_pbrs_curriculum`, `train_c_gym_pbrs_asp`). All 11 training scripts now log the iteration counter under a single uniform tag — the batch-size vs update-count confound discussed in thesis_plan.md §2.1 is now quantifiable from TensorBoard for every model. |
| 2026-07-12 | **P1 (orphan param files cleaned + auto-cleanup)** — 23 stale params files from earlier `gen_params.py` versions were present in `hpc/params/` but not referenced by `manifest.txt` (phase1_self, phase2_single, phase3b, phase4_pc, phase5_grid, etc.). These were removed. `gen_params.py.write()` now deletes all existing `.txt` files in the params dir before writing the fresh set, so no stale files can survive a re-generation. |
| 2026-07-12 | **Smoke-test campaign (pre-launch validation of the job-array campaign)** — Ran a staged smoke suite on Hábrók before the full 104-job campaign. **Passed:** (1) **Manifest integrity** — `gen_params.py` writes a clean 21-params-file set (+`manifest.txt`); `submit_all.sh --list` prints all 12 training + 9 validation phases with correct job counts, no errors. (2) **Training dry-runs** (all 8 training phases) — each `submit_all.sh --phase <p> --dry-run` emits a correct `[SUBMIT]` array spec with sane index list, mem/time tiers, throttle `%4`; no errors. (3) **Validation dry-runs** (all 9 validate phases) — reconcile correctly `[SKIP]`s phases with no ready checkpoints and submits only the one index (`phase1_anchor_validate [1]`) whose `model_best.pt` existed, confirming the CSV-existence + checkpoint-readiness reconcile (V5/V6). (4) **Single-agent train + resume** (`train_a_pbrs_simple.py`, `pbrsA_e256_i6188_s7`, 256 env) — container boots, GPU visible, trains to iter 132+, `Diagnostics/GradientUpdates` present; `scancel` at iter ~113 then re-submit resumed from `latest_checkpoint.pt` at the embedded authoritative iteration 100 (not 0), restored best-SR, no `[PPO LOAD] Shape mismatch` — verifies Fix P51/P74/P75, G2, G3, H4, R1a. (5) **Self-play train + checkpoints** (`train_e_pbrs_asp_dpose.py`, `pbrsE_e528_i3000_s7`, 528 env) — `[Init] Phase stagger applied`, `[Bob Update 0]`, iterates cleanly, creates `alice/`+`bob/`+`summary/`; `dpose L=0.07 thr=0.055` active. (6) **Disc subspace** (`train_f_pbrs_asp_disc.py`, `discF_e528_i3000_s7`, 528 env) — `char_length=0.0` collapses d_pose to position-only, `RotErr=0.0000rad` sustained across all iters (rotation correctly ignored), confirming the §14.11 orientation-zeroing subspace end-to-end. |
| 2026-07-12 | **Smoke-test fixes + findings** — Issues surfaced and resolved during the smoke campaign. **(SM1, gym dependency) BLOCKER FIXED** — `train_a_gym_pbrs_simple.py` crashed with `ModuleNotFoundError: No module named 'gym_pusht'`: the package was absent from both `curobo_overlay.img` (mounted by `train_gym.slurm`) and `gym_overlay.img`. Diagnosis: `gym_overlay.img` had the deps (pymunk 7.3, pygame 2.6, shapely 2.1, opencv) but not `gym_pusht`; the template also mounted the wrong overlay. **(SM2, gym fork version)** — after installing, `PushTEnv.__init__() got an unexpected keyword argument 'arena_width'`: the stock PyPI `gym_pusht` (baked into `isaac-lab.sif` at `_isaac_sim/kit/python/.../site-packages`) lacks the `arena_width` param that the local fork `/home3/s3426394/master_isaac/gym-pusht` adds. Fixed by installing the local fork into the overlay (must shadow the in-.sif copy; the container's Python is only reachable via `isaaclab.sh -p -m pip`, not bare `python3`/`pip3`). **(SM3, V100 incompatible) FINDING** — a `--gpus-per-node=v100:1` test crashed in 36s with `RuntimeError: CUDA error: no kernel image is available for execution on the device`: the `isaac-lab.sif` torch (cu128) ships no SM 7.0 kernels, so V100 (CC 7.0) is unusable. Compatible GPUs are RTX Pro 6000 (CC 12.0) and A100 (CC 8.0) only; never use untyped `--gpus-per-node=1` (routes to A100-or-V100). **(SM4, rogue resubmit chains) OPERATIONAL** — smokes given a short `--time` hit the wall, and the USR1 trap in the array templates auto-resubmitted full 23h chains on `gpumedium` (jobs `30108142`, `30108756`); these were cancelled. Correct campaign behaviour (`submit_all.sh` sets `JOB_TIME` explicitly), but for manual smokes: `scancel` at first `[Iter 1]` before the wall. **(SM5, measured resource sizing)** — Hábrók epilogue accounting across the smokes: single-agent 256-env RAM peak **6.5 G** / req 24 G, ~1.1 CPU-core avg / req 16; self-play 528-env RAM **5.71 G**, VRAM **13.16 G** / 96 G, GPU util 94%; disc 528-env RAM **5.63 G**, VRAM **14.80 G** / 96 G, util 95%. Node RAM is ~1.5 TB (resolves the wiki 192 G-vs-1.5 TB ambiguity). Conclusion: RAM tiers ~4× over-provisioned (24 G→12 G candidate for single-agent/self-play); GPU VRAM ~15% of the 96 G card (steady-state; the transient ~82 G seen via `nvidia-smi` was CUDA-compile scratch); gym is CPU-only (0% GPU, 0 VRAM, 0.9 G RAM). |
| 2026-07-12 | **Critical smoke expansion (train_h/g/i + 2048 + gym)** — **(SM6, train_h** `discF_e528_i3000_s7` taspH) smoke passed: `disc_dpose L=0.0 thr=0.05`, `RotErr=0.0000` sustained. Confirms H2/H3 (episode_manager path + group-consistency guard) and the time-based Alice reward on disc. **(SM7, train_g** `pbrsG_e528_i3000_s7` TASP T-block) smoke passed: `alice_reward_scale=0.5` active, `dpose L=0.07 thr=0.055`. **(SM8, train_i** `taspI_e528_i3000_s7` Bob-penalty) smoke passed: `--bob_time_penalty` active, `Reward/Bob/TimePenalty` logged. **(SM9, gym 64-env)** smoke passed after template fix (partition=regular, gym_overlay mount, fork install). **(SM10, 2048-env)** smoke queued — pending VRAM/RAM measurement. |
| 2026-08-11 | **Validation smoke-test campaign + fix** — Three validator spot-tests submitted: `validate_push.py` (single-agent, `pbrsA_e1024_i1547_s7`), `validate_push_asp.py` (ASP, `pbrsE_e528_i3000_s7`), and `validate_pusht_gym.py` (gym, `gymA_e64_p15_s42`). All three exited in 10–21 s with the same error: `ModuleNotFoundError: No module named 'asyncDualPlayPPO'`. **(V11, validator sys.path fix)** Root cause: training scripts sit at the package root (`asyncDualPlayPPO/`) and add `sys.path.append(..., "..")` (one level up to `master_isaac/`), making `asyncDualPlayPPO` importable. Validators are in `tests/` (one level deeper inside the package) — their `sys.path.append(..., "..")` resolved to `asyncDualPlayPPO/` itself, not its parent, so `import asyncDualPlayPPO` couldn't resolve. **Fix:** changed `sys.path.append` from `".."` to `"../.."` in all 6 validator scripts (`validate_push.py`, `validate_push_asp.py`, `validate_pusht_gym.py`, `validate_push_sac.py`, `validate_push_diffusion.py`, `validate_push_pusht.py`). `validate_pusht_gym.py` was missing the line entirely — added. The in-container import path is: container binds `master_isaac/asyncDualPlayPPO` → `/workspace/isaaclab/user_project/asyncDualPlayPPO`, so `asyncDualPlayPPO` is importable only from the parent `/workspace/isaaclab/user_project/`; `tests/` scripts need two levels to reach it. |
| 2026-07-12 | **Resource right-sizing** — **(R5, RAM)** based on SM5 measured peaks (single-agent 6.5 G, self-play 5.71 G, gym 0.9 G): `MEM_TIER` single/self 24G→12G (all env counts except 2048 which stays 48G pending smoke_2048), gym 16G→8G in `GYM_MEM`, validation 24G→12G in manifest. SLURM template `#SBATCH --mem` and `JOB_MEM` chain-resubmit defaults updated to match. 2048-env tier held at 48G. **(R6, CPU)** 16→8 deferred (low confidence; PPO-update single-core spikes could strand a packed node). **(R7, gym partition)** `train_gym.slurm` and `validate_gym.slurm` switched from `--partition=gpu / rtx_pro_6000:1` to `--partition=regular` with no GPU directive — gym is CPU-only (0% GPU util measured). Gym overlay mount corrected from `curobo_overlay.img` to `gym_overlay.img`. **(R8, liveness monitor)** `hpc/health_check.sh` (cron every 3h) reports running/pending counts, `.done` completions, ABORTED sentinels, TensorBoard activity, disk usage, and stalled (>6h) jobs. **(R9, slurm .out file hygiene)** — all 5 templates write `--output` to `/scratch/%u/slurm_logs/` instead of the project root (home directory quota risk: ~400 .out files × 50–200 MB = 20–80 GB). Health monitor auto-deletes `.out` files older than 7 days from that directory. `mkdir -p` in `_campaign.sh` campaign_start() and both validate templates ensures the directory exists before SLURM writes to it. |
| 2026-08-13 | **Campaign scoping — thesis-only focus** — Supervisor confirmed 2-seed CI is sufficient for Master thesis (not publication-level). All non-essential training cancelled: **11 running + 23 pending** `sa_train`/`sp_train` jobs removed via `scancel`, retaining only the 4 fast-track `fast_pbrsA`/`fast_pbrsB` 1024-env submissions (pbrsA s42, pbrsA s7, pbrsB s7, pbrsB s42). pbrsA s123 already done (`.done` on scratch). End-state: pbrsA 3-seed CI (s123/s42/s7) + pbrsB 2-seed CI (s7/s42), all at 1024 envs × 1547 iters. Deferred: A100 compat test (`30108818`, all A100 nodes drain/down — untestable); V100 confirmed incompatible (cu128 SM 7.0), RTX Pro 6000 remains the only verified-working GPU. |
| 2026-08-13 | **Validation-speed optimisations** — To get validation results within the thesis deadline, three changes applied to the validate templates and param generator. **(V12, max_pushes/max_tries)** `gen_params.py` now appends `--max_pushes 10 --max_tries 3` to every validate EXTRA field (lines 83, 90, 94); validate.slurm passes these via `$EXTRA` last on the command line, overriding the template's hardcoded `--max_pushes 30 --max_tries 20`. Per-job runtime reduced from ~3h (30×20=600 worst-case pushes) to ~1h (10×3=30 max pushes per scene × 30 scenes). **(V13, partition)** `validate.slurm` partition changed from `gpu` (lands on `gpumedium`, deep 23h queue) to `gpushort` (same RTX GPUs, shorter queue, lower fairshare impact); time reduced 06:00→03:00. **(V14, gym mem)** `validate_gym.slurm` mem reduced 32G→8G (measured peak 1.41G); manifest gym-validate time 04:00→02:00, mem 32G→8G. **(V15, launch)** All 8 done phases submitted under campaign `val_fast` (2026-08-13): 80 validation jobs total (73 Isaac on gpushort + 5 gym on regularshort + 2 batches covering 55+ .done experiments), throttled at `%4`. Queue snapshot at launch: 23 total gpushort jobs, 7 of yours PENDING + 3 RUNNING, gpushort idle GPUs on v100v2gpu[1,4,7] (V100 — unusable by cu128). Single-agent phases (phase1_anchor, phase2_ci single-agent lines) deferred until pbrsA/pbrsB fast-track finishes. |
...
### 14.10 Validation-path hardening (review items V1–V9)

A validation-path review surfaced nine items; verdicts and fixes:

| # | Item | Verdict | Fix |
|---|---|---|---|
| V1 | Disc PC validated on T-block scenes (ALL_TESTS has 0 disc scenes) | **Real blocker** | `DISC_TESTS` = 30 T-block scenes mirrored to disc (position-only, `goal_yaw=0`, `object_type=disc`); `--scene-set {all,disc,tblock}` on `validate_push_asp.py`; `phase4_pc` validator uses `--scene-set disc` |
| V2 | `model_best` chosen on training d_pose≤0.055, validated on combined gate | **Real** | `train_e`/`train_g` gate `model_best.pt` on combined SR (`pos<0.05 ∧ rot<0.2`); logged as `Metrics/Bob/CombinedSR`. `train_f` (disc) already position-only-consistent |
| V3 | d_pose runs scored on a looser gate than single-agent | **Non-issue** | Validator gate never uses `dpose_threshold`; T-block → `pos<0.05 ∧ rot<0.2`, disc → `pos<0.05`. Cross-model comparable already |
| V4 | OOM (137) treated as preemption → silent resubmit | **Real** | Templates drop the 137/143→resubmit branch; only the USR1 time-limit path (which sets `WILL_RESUBMIT`) resubmits, so OOM trips the brake |
| V5 | Validators never reconciled → full re-run on relaunch | **Real** | `submit_all.sh` reconciles validators by output-CSV existence |
| V6 | Validators hard-fail if only `latest_checkpoint.pt` exists | **Real (mild)** | `--allow-latest` on `submit_all.sh` + `validate*.slurm`: validate `latest_checkpoint.pt` into a separate `validation_latest/` subfolder (distinct filenames, excluded from collation); launcher submits a run only when a usable checkpoint exists |
| V7 | noge validation needs shape auto-detect | **Correct as-is** | `validate_push_asp.py` detects `pi_obj_dim=22` from checkpoint weights |
| V8 | Collator regex `…_s(seed)$` drops `_noge/_abc` runs | **Real** | Regex allows optional trailing `_<tag>`, folded into the model label; each ablation aggregates as its own CI row |
| V9 | Gym not validated/collatable | **Real (quantitative)** | New `validate_gym.slurm` (`validate_pusht_gym.py`, bob/agent auto-detect); `phase5_gym_validate` params; collator ingests gym `_e<envs>_p<pns>_s<seed>` CSVs |

Cross-model comparability is now guaranteed: every self-play headline number is a
`model_best` chosen on, and scored on, the same combined (0.05 m ∧ 0.2 rad) gate;
the disc PC is scored position-only on disc scenes. `latest_checkpoint`
validations are quarantined in `validation_latest/` so they never contaminate the
`collate_seeds.py` headline aggregation (which globs one level only).

### 14.11 Disc orientation-zeroing subspace + T-block→disc cross-eval

To make the disc a strict orientation-free **subspace** of the T-block obs (so a
T-block-trained policy sees a disc scene as an in-distribution "yaw-aligned
T-block", and the disc-native agent trains on that same subspace):

- **Shared transform** `reward_pbrs.dpose_and_zero_yaw(obs, robot_dim, obj_state_dim,
  goal_dim, char_length)` — writes the `[d_pose, bearing]` tail and, **iff
  `char_length == 0`**, zeros the object- and goal-yaw obs slots (indices
  `robot_dim+5` and `robot_dim+obj_state_dim+5`). Roll/pitch untouched (tip-over
  still reads them). The invariant `char_length==0 ⟺ yaw slots zeroed` is encoded
  in this one function so it cannot drift.
- **Wrapper** (`wrapper_push_asp.py`): `_get_push_obs` and `construct_bob_observation`
  both route their d_pose tail through `dpose_and_zero_yaw`, so rollout obs and ABC
  demos are byte-identical. E/G (char_length=0.07) keep live yaw; F/H (char_length=0)
  get the zeroed subspace automatically. A startup log tags the mode
  "[orientation-free subspace: obj/goal yaw zeroed]".
- **Validator** (`validate_push_asp.py`): the disc branch's manual `_build_obs`
  now applies the same `dpose_and_zero_yaw` (fixing a pre-existing tail mismatch —
  it previously emitted `[pos_dist, rot_dist]` instead of `[d_pose, bearing]`, so
  Model F/H were validated on a subtly wrong `bearing` slot). This makes disc-native
  validation and the cross-eval byte-identical to training.
- **Cross-eval phase `phase4_xeval`**: E and G `model_best` (Phase 3, 3 seeds each)
  validated on the disc scenes with `--char-length 0.0 --scene-set disc`. This is
  the actual T-block→disc transfer test. `gen_params.Suite.add_xeval` emits these
  as validation-only entries reusing each run's own checkpoint.
- **No CSV collision**: any disc-scene evaluation (`--scene-set disc`) writes to an
  `_sdisc`-tagged CSV (`validation_results_asp_sdisc.csv`), so a T-block model's
  cross-eval never overwrites its native T-block `validation_results_asp.csv`;
  `submit_all.sh` reconcile keys on the same suffix. Verified in a mock run:
  adding the native T-block CSV does **not** mark the `_sdisc` cross-eval done.
- **Cross-object safety** (checked): `ValidateEnvCfg` always instantiates both
  `target_object` (T-block) and `disc_object`, so disc scenes resolve for any
  checkpoint; Bob's architecture (incl. GoalEncoder / `pi_obj_dim`) is
  auto-detected from checkpoint weight shapes; `char_length`/`scene-set` are
  eval-time flags independent of the checkpoint's training value; obs is identical
  28-D so a T-block E/G policy loads and runs on disc scenes safely.
- **Retrain requirement**: Model F/H and the `phase4_pc` disc runs must be trained
  under the updated wrapper (they now train on the yaw-zeroed subspace) — land this
  before submitting `phase4_pc`; clear `.done` on any already-trained disc runs.
  E/G need no retrain (char_length=0.07 keeps live yaw). `phase4_xeval` is
  validation-only and needs no retrain.
