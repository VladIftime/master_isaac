...
- Bob reward: sparse `{+1/−1/+5}` + **dense per-push improvement** (Fix P53): `R = 12·Δd − 0.5·d_now + 1·Δyaw − 0.25·yaw_now` (all clamped) + phase-end progress reward `r = clamp(0.6·Δpos/init_pos + 0.4·Δrot/init_rot, −1, +1)` paid once at Bob termination. Dense reward has no completion bonus to avoid double-counting. Also -5 penalty for object-lifted/robot-through-table pushes (Fix P57c). Init errors captured on first Bob step. Gives 100% episode coverage — every Bob trial produces a grade plus per-push directional signal. `bob_timesteps` reduced 200→100 to halve credit-assignment horizon for single-object push task. (Dense per-step delta was tested v2–v5 and reverted per Fix P27 on the cuRobo per-step variant — the push-primitive dense reward is per-push, not per-step, and uses improvement deltas not potential shaping.)
