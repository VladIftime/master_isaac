...
def get_joint_patterns(side: str) -> list[str]:
    return _DEFAULT_JOINT_PATTERNS[side]
...
def build_rmpflow_action(
    ...
    if config_file is None:
        config_file = os.path.join(_pkg_root, "urdf", "cuMotion", f"rmpflow_config_{side}.yaml")
    if urdf_file is None:
        urdf_file = os.path.join(_pkg_root, "assets", "urdf", "ur_robotics", "ur5e", "ur5e_robotiq_140.urdf")
    if collision_file is None:
        collision_file = os.path.join(_pkg_root, "urdf", "cuMotion", f"lula_{side}.yaml")
...
def build_ik_action(
    ...
    **kwargs,
    ...
    if solver_type not in IK_BUILDERS:
        raise ValueError(f"Unknown IK solver '{solver_type}'. Options: {list(IK_BUILDERS)}")
