...
log_file = "/home/vlad/IsaacLab/vlad/master_isaac/asyncDualPlayPPO/logs/slurm-28170760-encoder_high.out"
...
with open(log_file, "r") as f:
    for line in f:
        ...
        if current_agent == "Alice":
            ...
            m_loss = re.search(
                r"Losses:\s+value=([-\d.]+)\s+\|\s+surrogate=([-\d.]+)", line
            )
            if m_loss:
            ...
            m_loss = re.search(
                r"Losses:\s+value=([-\d.]+)\s+\|\s+surrogate=([-\d.]+)\s+\|\s+ABC=([-\d.]+)",
                line,
            )
            if m_loss:
...
# Alice Rewards
axs[0, 0].plot(
    alice_iters[: len(alice_rewards)], alice_rewards, label="Alice Mean Reward"
)
...
# Alice Losses
axs[0, 1].plot(
    alice_iters[: len(alice_value_loss)], alice_value_loss, label="Value Loss"
)
axs[0, 1].plot(
    alice_iters[: len(alice_surrogate_loss)],
    alice_surrogate_loss,
...
# Bob Success Rate
axs[1, 0].plot(
    bob_iters[: len(bob_success_rate)],
    bob_success_rate,
    label="Success Rate",
    color="green",
)
...
axs[2, 0].plot(
    bob_iters[: len(bob_surrogate_loss)], bob_surrogate_loss, label="Surrogate Loss"
)
