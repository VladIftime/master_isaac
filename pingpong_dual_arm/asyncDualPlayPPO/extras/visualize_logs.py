...
def parse_log(filepath):
    ...
    for m in alice_loss_pattern.finditer(text):
        ...
        if idx not in [
            alice["value_loss"][i]
            for i in range(len(alice["value_loss"]))
            if i < len(alice["update"]) and alice["update"][i] == idx
        ]:
            pass
...
def plot_runs(runs, labels, save_path=None):
    ...
    fig_bob.suptitle(
        "Bob's Training Progress", fontsize=16, fontweight="bold", color="#d62828"
    )

    for i, (alice, bob, bob_rewards, label) in enumerate(
        zip(*zip(*[(r, l) for r, l in zip(runs, labels)]))
        if False
        else zip(
            [r[1] for r in runs], [r[2] for r in runs], [r[0] for r in runs], labels
        )
    ):
    ...
    for i, ((alice, bob, bob_rewards), label) in enumerate(zip(runs, labels)):
        ...
        axes_bob[0, 2].plot(
            x, bob["abc_loss"], "-o", color=c, label=label, markersize=3, linewidth=1.5
        )
        axes_bob[0, 2].set_title("ABC Loss")
        axes_bob[0, 2].axhline(
            y=-1.0,
            color="#d62828",
            linestyle="--",
            alpha=0.5,
            label="Broken (-1.0)" if i == 0 else None,
        )

        axes_bob[1, 0].plot(
            x, bob["pos_err"], "-o", color=c, label=label, markersize=3, linewidth=1.5
        )
        axes_bob[1, 0].set_title("Position Error (m)")
        axes_bob[1, 0].set_xlabel("Bob Update")

        axes_bob[1, 1].plot(
            x, bob["rot_err"], "-o", color=c, label=label, markersize=3, linewidth=1.5
        )
    ...
    fig_alice.suptitle(
        "Alice's Training Progress", fontsize=16, fontweight="bold", color="#003049"
    )
    ...
    for i, ((alice, bob, bob_rewards), label) in enumerate(zip(runs, labels)):
        ...
        if alice["value_loss"]:
            ...
            axes_alice[1].plot(
                ax_x, alice["value_loss"], "-", color=c, label=label, linewidth=1.2
            )
        ...
        if alice["surrogate_loss"]:
            ...
            axes_alice[2].plot(
                ax_x, alice["surrogate_loss"], "-", color=c, label=label, linewidth=1.2
            )
    ...
    if any_rewards:
        ...
        fig_rew.suptitle(
            "Bob Reward Events", fontsize=14, fontweight="bold", color="#2a9d8f"
        )
        for i, ((alice, bob, bob_rewards), label) in enumerate(zip(runs, labels)):
            if bob_rewards:
                ax_rew.hist(
                    ...
                    label=f"{label} ({len(bob_rewards)} events)",
                    edgecolor="white",
                    linewidth=0.5,
                )
...
def main():
    ...
    parser.add_argument(
        "--labels", nargs="+", default=None, help="Labels for each file"
    )
    parser.add_argument(
        "--save", type=str, default=None, help='Save prefix (e.g. "plots/run")'
    )
    ...
    for f in args.files:
        ...
        print(
            f"[{p.name}] Alice updates: {len(alice['update'])}, Bob updates: {len(bob['update'])}, Bob reward events: {len(bob_rewards)}"
        )
