...
namespace OgnSendMessageBusEventAttributes
{
namespace inputs
{
...
using namespace OgnSendMessageBusEventAttributes;
namespace IOgnSendMessageBusEvent
{
// Pushes a named event to the Application Message Bus. The payload of the event is
// constructed from any dynamic input attributes that have been added to this node and
// consists of a dictionary whose key is the name of the attribute, with the 'inputs:'
// prefix stripped from it, with a value equal to the raw pointer to the attribute's
// data in Fabric. If the attribute has a type that is not able to be converted to this
// form then a warning is posted for the node but the execution continues.
class OgnSendMessageBusEventDatabase : public omni::graph::core::ogn::OmniGraphDatabase
{
public:
    template <typename StateInformation>
    CARB_DEPRECATED("sInternalState is deprecated. Use sSharedState or sPerInstanceState instead")
    static StateInformation& sInternalState(const NodeObj& nodeObj, InstanceIndex index = {kAuthoringGraphIndex}) {
        return sm_stateManagerOgnSendMessageBusEvent.getState<StateInformation>(nodeObj.nodeHandle, index);
    }
    template <typename StateInformation>
    static StateInformation& sSharedState(const NodeObj& nodeObj) {
        return sm_stateManagerOgnSendMessageBusEvent.getState<StateInformation>(nodeObj.nodeHandle, kAuthoringGraphIndex);
    }
    template <typename StateInformation>
    ...
    static StateInformation& sPerInstanceState(const NodeObj& nodeObj, GraphInstanceID instanceId) {
        return sm_stateManagerOgnSendMessageBusEvent.getState<StateInformation>(nodeObj.nodeHandle, instanceId);
    }
    template <typename StateInformation>
    ...
    static ogn::StateManager sm_stateManagerOgnSendMessageBusEvent;
    static std::tuple<int, int, int>sm_generatorVersionOgnSendMessageBusEvent;
    static std::tuple<int, int, int>sm_targetVersionOgnSendMessageBusEvent;
    ...
    OgnSendMessageBusEventDatabase(NodeObj const& nodeObjParam)
    : OmniGraphDatabase()
    ...
    OgnSendMessageBusEventDatabase(GraphContextObj const&, NodeObj const& nodeObjParam)
    : OgnSendMessageBusEventDatabase(nodeObjParam)
    {}

    //Main constructor
    OgnSendMessageBusEventDatabase(GraphContextObj const* contextObjParam, NodeObj const* nodeObjParam, size_t handleCount)
    : OmniGraphDatabase()
    , inputs{m_offset.index}
    , outputs{m_offset.index}
    {
        ...
        NodeObj const& nodeObj = abi_node();
        {
            auto inputDataHandles0 = getAttributesR<
                ...
                >(contextObj, nodeObj.nodeContextHandle, std::make_tuple(
                    inputs::eventName.m_token, inputs::execIn.m_token
                )
...
public:
    static void initializeType(const NodeTypeObj& nodeTypeObj)
    {
        ...
        if( ! iTokenPtr ) {
            CARB_LOG_ERROR("IToken not found when initializing omni.graph.action.SendMessageBusEvent");
            return;
        }
        ...
        iNodeType->setMetadata(nodeTypeObj, kOgnMetadataUiName, "Send MessageBus Event");
        iNodeType->setMetadata(nodeTypeObj, kOgnMetadataCategories, "graph:action,event");
        iNodeType->setMetadata(nodeTypeObj, kOgnMetadataDescription, "Pushes a named event to the Application Message Bus. The payload of the event is constructed from any dynamic input attributes that have been added to this node and consists of a dictionary whose key is the name of the attribute, with the 'inputs:' prefix stripped from it, with a value equal to the raw pointer to the attribute's data in Fabric. If the attribute has a type that is not able to be converted to this form then a warning is posted for the node but the execution continues.");
    ...
    static void initialize(const GraphContextObj&, const NodeObj& nodeObj)
    {
        ...
        attr.iAttribute->setMetadata(attr, kOgnMetadataDescription, "The name of the custom event to be sent.");
    ...
    {
        sm_stateManagerOgnSendMessageBusEvent.removeState(nodeObj.nodeHandle, instanceID);
    }
    bool validate() const {
        return validateNode()
...
ogn::StateManager OgnSendMessageBusEventDatabase::sm_stateManagerOgnSendMessageBusEvent;
std::tuple<int, int, int> OgnSendMessageBusEventDatabase::sm_generatorVersionOgnSendMessageBusEvent{std::make_tuple(1,79,2)};
std::tuple<int, int, int> OgnSendMessageBusEventDatabase::sm_targetVersionOgnSendMessageBusEvent{std::make_tuple(2,184,5)};
}
using namespace IOgnSendMessageBusEvent;
#define REGISTER_OGN_NODE() \
namespace { \
    ogn::NodeTypeBootstrapImpl<OgnSendMessageBusEvent, OgnSendMessageBusEventDatabase> s_registration("omni.graph.action.SendMessageBusEvent", 1, "omni.graph.action_nodes"); \
}
