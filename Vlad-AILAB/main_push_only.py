#!/usr/bin/env python3
"""
UR5e Push Manipulation Simulation

This script demonstrates a UR5e robot performing push manipulation.
It creates a cube at the center of the platform and pushes it.
Height maps are saved at two stages: before pushing and after pushing.
...
Example usage:
  python main_push_only.py                    # Run without visualizations
  python main_push_only.py --display-images   # Run with visualizations enabled
"""
...
parser = argparse.ArgumentParser(description="UR5e Push Manipulation Simulation")
parser.add_argument(
    "--display-images",
    action="store_true",
    default=False,
    ...
    if my_world.is_playing():
        ...
        if push_controller.is_done() and push_started and not heightmap_after_push_saved:
            ...
            if my_world.current_time_step_index > 0:  # Ensure we're past initial steps
                ...
                print(f"\n{'='*60}")
                print(f"PUSH SIMULATION COMPLETED")
                print(f"{'='*60}")
