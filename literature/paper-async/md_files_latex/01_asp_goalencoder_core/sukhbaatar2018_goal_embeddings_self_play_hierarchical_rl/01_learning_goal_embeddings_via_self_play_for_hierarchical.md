...
$$a_t^B = \pi_B(s_t^B, s^*) = \pi'_B(s_t^B, E(s^*, s_t^B)).$$
...
This vector is then fed to Bob's goal policy  $\pi_B'$ , which converts it to an action on the environment
