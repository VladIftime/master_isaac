...
\begin{frame}[t]{Baseline — Fractional Reward at Scale}
  ...
  \begin{tabular}{lccc}
    ...
    \midrule
    \rowcolor{hlgreen!15}
    PPO single-agent & \textcolor{hlgreen}{\textbf{83.3\%}} & 100\% & 75\% \\
...
\begin{frame}[t]{ASP vs TASP training}
  ...
  {\scriptsize ASP-dPose (outcome) vs TASP-dPose (time): displacement (top), valid-goal density (bottom). Bob solves few valid goals (3.9\% vs 5.5\%).}
...
\begin{frame}[t]{Conclusion — Environment Efficiency (RQ1)}
  \begin{block}{\textbf{RQ1 — PBRS reduces parallel-environment requirements}}
    \small
    PBRS achieves 80.0\% SR at 528 parallel envs — 4$\times$ fewer than the fractional reward requires (2{,}048 envs) for the same performance (83.3\%). Dense, correctly-signed, bounded rewards on every push. Policy invariance (Ng et al.\ 1999) holds with stochastic PhysX.
  \end{block}
