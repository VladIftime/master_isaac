...
\begin{frame}[t]{Per-Scene Outcomes — Isaac vs Gym}
  ...
  \vspace{2pt}
  {\small Green = scene solved (any of 20 trials), Red = failed }
...
\begin{frame}[t]{Discussion — What the Validation Suite Reveals}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.34\textwidth}
      ...
      \begin{itemize}
        \item Position-only scenes: solved by both single-agent models (100\%).
        \item Pos+rot scenes: the bottleneck — PPO-PBRS 70\%, self-play $\le$10\%.
      \end{itemize}
    \end{column}
...
\begin{frame}[t]{ASP — The Empirical Result}
  ...
  {\small
  \begin{itemize}
    \item Time-based Alice (TASP-dPose) is the best variant — 2.5$\times$ better than outcome-based, but still \textbf{4.8$\times$ below} single-agent
...
\begin{frame}[t]{Key Result — PBRS Substitutes for Curriculum}
  ...
  \begin{block}{The Substitution Finding (RQ3)}
    \footnotesize
    \begin{itemize}
      \item \textbf{Curriculum adds no value at this scale:} PPO-Curriculum 76.7\% vs PPO-PBRS 80.0\% — 3.3pp gap \textbf{not significant} (p\,$>$\,0.05). PBRS \emph{substitutes} for the staged curriculum.
      \item \textbf{Regime-dependent:} holds at Isaac batch 7{,}920; at gym batch 960, curriculum \emph{does} help (50\% vs 10\%).
      \item \textbf{Self-play (4.8--11.9$\times$ worse) helps in neither regime.} PBRS is the dominant lever — invest in reward design first.
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}[t]{Conclusion — Reward \& Self-Play (RQ1--2)}
  \begin{block}{\textbf{RQ1 — PBRS enables efficient on-policy pushing}}
    \small
  ...
  \vspace{2pt}
  \begin{block}{\textbf{RQ2 — Self-play fails structurally on this task}}
    \small
    Five ASP configurations: \textbf{no variant exceeds 16.7\%}. Single-agent ceiling: 80.0\%. Self-play and single-agent cost the same per iteration — the gap is structural, not a budget artifact. Time-based Alice partially mitigates (+10pp) but cannot close the 4.8$\times$ gap.
  \end{block}

  \vfill\litfooter{[Ng et al.\ 1999]~[Grze\'s 2017]~[Plappert et al.\ 2021]~\textbf{[Sukhbaatar et al.\ 2018]}}
\end{frame}

% ~~~~~ SPEAKER NOTES ~~~~~
\note[itemize]{
\item RQ1: PBRS at 528 envs matches the original reward at 2{,}048 envs — 4$\times$ more environment-efficient. Correctly-signed, bounded, Markovian rewards enable stable on-policy GAE
\item RQ2: No ASP configuration exceeds 16.7\% (TASP-dPose) against 80\% single-agent. Five configurations tested — original reward (0\%), outcome-based (6.7\%), time-based (16.7\%), symmetric Bob-penalty (0\%), gym-pusht (13.3\%)
\item Self-play and single-agent have identical per-iteration wall-clock (\textasciitilde0.022 it/s) — the 4.8$\times$ gap is structural, not a budget artifact
\item Time-based Alice rewards solvable goals, partially mitigating the adversarial distribution shift, but cannot close the fundamental gap
}


\begin{frame}[t]{Conclusion — Curriculum Substitution (RQ3)}
  \begin{block}{\textbf{RQ3 — PBRS substitutes for curriculum at sufficient batch size}}
    \small
    PPO-PBRS (80.0\%) vs PPO-Curriculum (76.7\%): 3.3pp gap is \textbf{not statistically significant} (p\,$>$\,0.05). Substitution is \textbf{regime-dependent}:
    \begin{itemize}
      \item Isaac (batch 7{,}920): PBRS alone suffices — curriculum adds no value
      \item Gym (batch 960): curriculum helps (50\% vs 10\%) — PBRS gradient degraded by small-batch variance
    \end{itemize}
  \end{block}

  \vfill\litfooter{[Devlin \& Kudenko 2012]~[Grze\'s 2017]~[Narvekar et al.\ 2020]}
\end{frame}

% ~~~~~ SPEAKER NOTES ~~~~~
\note[itemize]{
\item RQ3: PPO-PBRS 80.0\% vs PPO-Curriculum 76.7\% — 3.3pp gap not statistically significant (p>0.05, SE diff=±10.6pp). A well-shaped reward substitutes for the staged curriculum
\item Regime-dependent: at Isaac batch 7{,}920 the curriculum is redundant; at gym batch 960 it helps (50\% vs 10\%). The substitution boundary depends on PPO+LSTM gradient variance
\item Practical implication: invest in reward design first — it is the higher-leverage intervention. Add curriculum only when batch constraints force it
}
...
\begin{frame}[t]{Limitations \& Future Work}
  ...
  \begin{enumerate}
    ...
    \item \textbf{RQ3 substitution is regime-dependent} — PBRS substitutes at batch 7{,}920; curriculum helps at batch 960.\\
