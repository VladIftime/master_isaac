...
\begin{frame}[t]{Research Questions}
  \begin{block}{Three Research Questions}
    \begin{enumerate}
      \item \textbf{RQ1:} Can potential-based reward shaping reduce the number of parallel environments required for on-policy learning of a multi-objective planar pushing task on a simulated robotic arm?
      \item \textbf{RQ2:} Given a well-shaped PBRS reward, does modifying the training dynamics — via sequential objective staging or adversarial goal generation — improve final task performance over a direct single-agent approach in a multi-objective planar pushing task on a simulated robotic arm?
      \item \textbf{RQ3:} Does asymmetric self-play enable the learning of individual position and orientation skills in a multi-objective planar pushing task on a simulated robotic arm?
    \end{enumerate}
  \end{block}
...
% ~~~~~ SPEAKER NOTES ~~~~~
\note[itemize]{
\item RQ1: PPO with original fractional reward at 2{,}048 envs reaches 80\%. PPO-PBRS reaches 80\% at 528 envs — 4$\times$ fewer parallel environments. PBRS is more environment-efficient and theoretically principled.
\item RQ2: PPO-Curriculum at 76.7\% is statistically tied with PPO-PBRS at 80.0\% (n.s.) — sequential staging adds no value at this batch size. Self-play variants at 6.7--16.7\% — 4.8--11.9$\times$ worse. Adversarial goal generation degrades performance.
\item RQ3: Bob learns individual position (54\%) and rotation (45\%) under PBRS-ASP, but the combined gate caps at 10\% — 2.5$\times$ below the independence product. Under the original fractional reward, nothing is learned (1.3\% per-axis, 0.07\% combined). ASP teaches skills it cannot compose.
\item Five models total: PPO-Baseline (orig.\ reward, 2{,}048 envs), PPO-PBRS, PPO-Curriculum, ASP-dPose, TASP-dPose (528 envs each). \texttt{\textasciitilde}3000 iterations per PBRS model, RTX Pro 6000.
}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\textwidth}
      ...
      \begin{enumerate}
        ...
        \item \textbf{Fair test} — same batch size and budget as the winning single-agent, yet self-play reaches only 6.7--16.7\% vs 80\% (4.8$\times$ gap). The gap is structural.
      \end{enumerate}
    \end{column}
    \begin{column}{0.50\textwidth}
      \centering
...
\begin{frame}[t]{Multi-Model Validation}
  ...
  \vspace{2pt}
  {\small \textbf{PPO-PBRS vs PPO-Curriculum:} 80.0\% vs 76.7\% — \textbf{not statistically significant} (p\,$>$\,0.05). \textbf{Self-play:} 4.8--11.9$\times$ below single-agent.  All models evaluated under identical protocol (30 scenes, 20 trials, 30 pushes).}

  \vspace{2pt}
  {\scriptsize \textbf{Protocol note:} Scene SR (reported) counts a scene solved if $\ge$1 of 20 trials passes — standard for robotics manipulation evaluation. Per-attempt SR is lower: PPO-PBRS 66\%, PPO-Curriculum 68\%. The 14pp gap reflects 6 rotation-dominant scenes that systematically fail all 20 attempts, while position-only scenes succeed reliably. See Appendix L for protocol details.}
\end{frame}
...
\item Key nuance: at the small gym batch, PPO-Curriculum (50\%) far exceeds PPO-PBRS (10\%) — curriculum helps when gradients are noisy; at the large Isaac batch they tie. The substitution is regime-dependent
...
\begin{frame}[t]{Per-Scene Outcomes — Isaac vs Gym}
  ...
  \vspace{2pt}
  {\small Colour: red (0 trials passed) $\to$ yellow (10) $\to$ green (all 20). Numbers = successful trials per scene out of 20.}
...
\begin{frame}[t]{Conclusion — Environment Efficiency (RQ1)}
  \begin{block}{\textbf{RQ1 — PBRS reduces parallel-environment requirements}}
    \small
    PBRS achieves 80.0\% SR at 528 parallel envs — 4$\times$ fewer than the fractional reward requires (2{,}048 envs) for the same performance. Dense, correctly-signed, bounded rewards on every push. Policy invariance (Ng et al.\ 1999) holds with stochastic PhysX.
  \end{block}
...
% ~~~~~ SPEAKER NOTES ~~~~~
\note[itemize]{
\item RQ1: PPO-PBRS at 528 envs matches PPO-Baseline at 2{,}048 envs — both reach 80.0\% validation SR
...
\item The policy invariance theorem is supported empirically: PBRS does not alter the optimal policy, only the learning speed
}


\begin{frame}[t]{Conclusion — Curriculum \& Self-Play (RQ2)}
  \begin{block}{\textbf{RQ2 — Staging adds no value; self-play degrades performance}}
    \small
    \textbf{Sequential objective staging} (PPO-Curriculum 76.7\%) is statistically tied with single-agent PBRS (80.0\%, n.s.).\\
    \textbf{Adversarial goal generation}: five self-play configurations across three simulators yield 6.7--16.7\% SR — 4.8--11.9$\times$ below single-agent. The non-stationary goal distribution prevents stable on-policy learning.
  \end{block}

  \vspace{4pt}
  {\small \textbf{The substitution finding:} PBRS \emph{substitutes} for an explicit curriculum at this batch size — curriculum adds 3.3pp (n.s.). This is \textbf{regime-dependent}: at gym batch 960, curriculum helps (50\% vs 10\%). Self-play (4.8--11.9$\times$ worse) helps in neither regime. Practical implication: invest in reward design first.}

  \vfill\litfooter{[Plappert et al.\ 2021]~\textbf{[Sukhbaatar et al.\ 2018]}~[Narvekar et al.\ 2020]}
\end{frame}

% ~~~~~ SPEAKER NOTES ~~~~~
\note[itemize]{
\item RQ2: PPO-Curriculum 76.7\% vs PPO-PBRS 80.0\% — 3.3pp gap not statistically significant (p>0.05, SE diff=±10.6pp). Sequential staging adds no net value at this batch size
\item Self-play at 6.7–16.7\% across five configurations: original fractional reward (0\%), outcome-based PBRS (6.7\%), time-based PBRS (16.7\%), symmetric Bob-penalty (0\%), gym-pusht (13.3\%)
\item Time-based Alice partially mitigates (+10pp over outcome-based) but cannot close the 4.8× gap
\item Self-play and single-agent cost the same per iteration — the gap is structural, not a budget artifact
}


\begin{frame}[t]{Conclusion — Individual Skills Under Self-Play (RQ3)}
  \begin{block}{\textbf{RQ3 — ASP teaches individual skills it cannot compose}}
    \small
    Under PBRS-ASP, Bob learns per-axis position (54\%) and rotation (45\%) individually — but the combined gate caps at 10\%, 2.5$\times$ below the independence product (24--25\%). Under the original fractional reward, nothing is learned (1.3\% per-axis, 0.07\% combined). The adversarial goal distribution prevents Bob from simultaneously satisfying coupled success criteria.
  \end{block}
...
% ~~~~~ SPEAKER NOTES ~~~~~
\note[itemize]{
\item RQ3: Training data from TensorBoard events — Bob PositionSR 54\%, RotationSR 45\%, Combined SR 10\%. Independence product: 24--25\%
\item Under the original fractional reward: PositionSR 1.3\%, Combined 0.07\% — no learning at all
\item PBRS provides enough gradient for per-axis learning, but the adversarial distribution prevents composing these skills into joint achievement
