...
\begin{frame}[t]{Isaac Lab — the Right Venue for Self-Play}
  ...
  \begin{tabular}{lcc}
    ...
    gym self-play (1 CPU)   & 22.4 & $\sim$12.4 h \\
    \bottomrule
  \end{tabular}
  ...
  \textbf{Three pillars:}
  \begin{itemize}
    \item \textbf{GPU-batched} — 528 envs/step gives 7,920-sample PPO batches; the self-play objective needs a large batch for stable gradients.
    \item \textbf{Robotic fidelity} — UR5e + 3D contact + cuRobo IK = the actual SE(2) task; gym‑pusht is a 2D diagnostic.
    \item \textbf{Time-to-scale} — $\sim$10M pushes in $\sim$16h on GPU vs $\sim$5 days on CPU.
  \end{itemize}

  \vspace{4pt}
  \textbf{Key result:} Isaac gives self-play its best shot at a budget matched to the winning single-agent — and it \textbf{still} reaches only 6.7--16.7\% (4.8$\times$ gap).
\end{frame}
