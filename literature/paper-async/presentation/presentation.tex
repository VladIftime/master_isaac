  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\textwidth}
      \begin{enumerate}
        ...
        \item \textbf{Implementation} \\ {\scriptsize four models: PPO-PBRS, PPO-Curriculum, ASP-dPose, TASP-dPose}
      \end{enumerate}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{enumerate}
...
\item Background covers the five literature areas; Implementation describes the four models; Evaluation defines the protocol; Results and Discussion present and interpret the validation outcomes
...
\begin{frame}[t]{Related Work — Reward Shaping (PBRS)}
  \textbf{Potential-based reward shaping} (Ng, Harada \& Russell 1999):
  ...
  \begin{itemize}
    ...
    \item \textbf{Episodic validity} (Grze\'s 2017): $\gamma_{\text{shaping}}{=}1$ is safe — $\sum F=\Phi(s_T)-\Phi(s_0)$ is bounded.
    \item \textbf{Dynamic / arbitrary potentials} (Devlin \& Kudenko 2012; Harutyunyan et al.\ 2015) — basis for our exponential $\Phi$.
  \end{itemize}
...
\begin{frame}[t]{Related Work — Curriculum \& Asymmetric Self-Play}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.50\textwidth}
      ...
      \begin{itemize}
        \item \textbf{Surveys} (Narvekar et al.\ 2020; Portelas et al.\ 2020)
        \item \textbf{Reverse / goal} (Florensa et al.\ 2017): start near the goal, expand outward
        \item \textbf{Precision} (Luo et al.\ 2020): gradually tighten the success tolerance
      \end{itemize}
    ...
    \begin{column}{0.50\textwidth}
      ...
      \begin{itemize}
        \item \textbf{ASP} (Sukhbaatar et al.\ 2018; Plappert et al.\ 2021): Alice/Bob adversarial game + ABC
        \item \textbf{Time‑based Alice} (Sukhbaatar 2018): $R_A\propto t_B\!-\!t_A$, self‑regulating
        \item \textbf{Regret‑based} (Dennis et al.\ 2020, PAIRED; Campero et al.\ 2021, AMIGo)
      \end{itemize}
...
\begin{frame}[t]{Related Work — Geometry \& Mechanics of Planar Pushing}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.56\textwidth}
      ...
      \begin{itemize}
        \item \textbf{Quasi-static pushing} (Mason 1986; Lynch \& Mason 1996): motion set by the friction limit surface.
        \item \textbf{Limit-surface normality} (Goyal et al.\ 1991; Howe \& Cutkosky 1996): object twist $\mathbf{t}\propto\nabla f(\mathbf{w})$ — \emph{every} push couples translation and rotation.
      \end{itemize}
      \textbf{SE(2) geometry}
      \begin{itemize}
        \item \textbf{Lie-group pose distance} (Park 1995; Urain et al.\ 2023): a geodesic metric fusing translation and rotation with characteristic length $L$.
        \item \textbf{Equivariant manipulation} (van der Pol et al.\ 2020; Huang et al.\ 2022): exploiting SE(2)/SO(2) symmetry.
      \end{itemize}
      \textbf{Our use:} limit‑surface coupling explains why the pos+rot gate is hard; the SE(2) $d_{\text{pose}}$ provides a unified reward for the self‑play variants.
    \end{column}
    \begin{column}{0.42\textwidth}
      \centering\includegraphics[width=0.95\textwidth]{figures/diagram_limit_surface.png}
    \end{column}
  \end{columns}
...
\begin{frame}[t]{The Four Models at a Glance}
  \centering\small
  \begin{tabular}{lll}
    \toprule
  ...
  \vspace{6pt}
  {\small All four share the same PPO+LSTM backbone, object-relative push primitive, 528 envs, and $\sim$3000 iterations. They differ only in \textbf{reward shape} and \textbf{training-goal generation}.}
...
\item \textbf{Confounded ablation:} Self-play vs single-agent differ on $\ge 7$ axes: agent count, goal distribution, episode budget, GoalEncoder, ABC, historical pool, update frequency. 4.8$\times$ SR gap cannot be attributed to any single lever (see Slide 9c).
}
...
\begin{frame}[t]{Results — Per-Test Error (Single-Agent Models)}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.5\textwidth}\centering
      \includegraphics[width=1.0\textwidth]{figures/errors_ppo_pbrs.png}\\[2pt]
      {\scriptsize PPO-PBRS — 80.0\% (24/30)}
    \end{column}
    \begin{column}{0.5\textwidth}\centering
      \includegraphics[width=1.0\textwidth]{figures/errors_ppo_curriculum.png}\\[2pt]
      {\scriptsize PPO-Curriculum — 76.7\% (23/30)}
    \end{column}
  \end{columns}
  \vspace{2pt}
...
\begin{frame}[t]{Results — Per-Test Error (Self-Play Models)}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.5\textwidth}\centering
      \includegraphics[width=1.0\textwidth]{figures/errors_tasp_dpose.png}\\[2pt]
      {\scriptsize TASP-dPose — 16.7\% (5/30)}
    \end{column}
    \begin{column}{0.5\textwidth}\centering
      \includegraphics[width=1.0\textwidth]{figures/errors_asp_dpose.png}\\[2pt]
      {\scriptsize ASP-dPose — 6.7\% (2/30)}
    \end{column}
  \end{columns}
  \vspace{2pt}
