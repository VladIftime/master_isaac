...
budget. This thesis isolates *why*, by identifying the dimensions on which the present
regime differs from the successful ones and testing each dimension directly. The
...
1. **Primary (ASP failure mechanism).** ASP learns the individual position and
   ...
   combined diagnostic, the value-bias measurement, and the ABC clip-saturation
   measurement.
2. **Secondary (regime contrast).** A structured comparison of this regime against
   ...
   family) along five dimensions: goal coupling, reward formulation, imitation signal,
   goal representation, and compute scale. Each dimension is both a literature claim
   and an experiment.
...
- Figures: generated by the analysis pipeline in `final_results_thesis/analysis/`
  (`plot_results.py`, `plot_presentation_figs.py`, `make_error_figure.py`,
  `val_collate.py`, `extract_tb_data.py`) into
  `final_results_thesis/analysis/plots_table/` (PNG + PDF) and copied into the
  thesis `images/` directory keeping original filenames. Simulation renders
  live in `images/sim_pics/`. Legacy presentation figures remain in
  `literature/paper-async/presentation/figures/` as historical reference.
...
develop across the regime dimensions.
...
writing to enforce this. Ledger entry: regime-contrast terms are
\emph{dimensions} ("five candidate dimensions", "single-dimension isolation"),
never "axes"; "per-axis" is reserved for the position/orientation objectives
(per-axis success rates).
...
- State the sample size and seed count at every quantitative claim: 30 held-out
  scenes, combined gate `pos < 0.05\,m` AND `rot < 0.2\,rad`, and the seed count
  (mean ± 95\,\% confidence interval). The evaluation budget itself is stated
  once, per rule §1.8.
- Both validators sample actions from the policy (not arg-max), so every attempt
  is a genuine draw for every model; single-agent and self-play are compared
  under one protocol.
...
- ASP differs from single-agent on several dimensions at once; state which single-dimension
...
### 1.8 Evaluation-setup placement (single source of truth)

- The evaluation set-up — the 30 held-out scenes, the attempts per scene and
  pushes per attempt of both the abbreviated and the full evaluation, and the
  cross-simulator budget — is stated exactly once, in the validation-protocol
  section of the Experiments chapter (`sec:val_protocol`). The cross-simulator
  budget sits with the cross-environment protocol (`sec:cross_env`), and
  run-exclusion notes (crashed seeds) with the training configuration
  (`sec:train_config`).
- Figure captions, table captions, and figure-embedded source notes must not
  restate the budget and must not use campaign names ("fast pass", "full pass",
  "best-of-20"). They say "held-out 30-scene suite" (or "held-out scenes") and,
  where the protocol matters, cross-reference the protocol section.
- This rule is enforced on both the `.tex` captions and the source notes inside
  the figure-generation scripts; every regeneration must re-check it.
...
### 2.2 The regime-contrast dimensions (the "why others succeeded" backbone)

Each dimension is a hypothesis for the ASP failure and a claim about why prior work
succeeded. The literature review argues the dimension; the campaign tests it.
...
### 2.3 Mandatory code fixes before the campaign is valid
...
Figures are generated by the analysis pipeline in `final_results_thesis/analysis/`
and copied into the thesis `images/` directory keeping original filenames
(§6.4 lists the scripts). Simulation renders live in `images/sim_pics/` and are
referenced with the `sim_pics/` prefix. The legacy presentation figures in
`literature/paper-async/presentation/figures/` were generated from the earlier
single-seed run set; anything carried over from there is regenerated from the
campaign outputs (seed means with confidence intervals) before final submission.
...
| Regime-contrast table (the five dimensions) | 2 Related Work | Why prior ASP succeeded, why this fails | literature + campaign |
...
| `asp_vs_tasp_training.png` | 5 Results (RQ2 reward) | Alice displacement + goal validity over training, E vs G |
| `per_scene_errors_3models.png` | 5 Results (Baseline) | Per-scene error bars, PBRS/Curriculum/ASP-dPose |

> **Regeneration status (2026-08-13):** `p3_heatmap.png`, `plot3_failure_taxonomy.png`,
> `asp_vs_tasp_training.png`, and `per_scene_errors_3models.png` have been
> regenerated from the campaign validation set
> (`final_results_thesis/analysis/val_data/validation_results_130826`) by
> `plot_presentation_figs.py` and `make_error_figure.py`, and the thesis `images/`
> copies now carry campaign data. The remaining §3.2 assets still hold
> single-seed data and stay on the regeneration list.

### 3.3 Simulation renders (`images/sim_pics/`)

| Asset | Chapter | Role |
|---|---|---|
| `isaac_enviroment_robot_setup.png` | 3 Methods §3.1 (`fig:env_render`) | Rendered environment: UR5e, tabletop, T-block, goal ghost |
| `isaac_fullsimview_push_t_task_20_parallel_env.png` | 3 Methods §3.1 (`fig:parallel_envs`) | GPU-batched parallelism; caption notes 20 shown vs 256–2{,}048 trained |
| `isaac_push_t_asp_red-alice_blue-bob.png` | 3 Methods §3.6 (`fig:asp_render`) | Rendered ASP loop; Alice red, Bob blue |
| `isaac_birdview_disc_asp_alice-red_bob-blue.png` | 3 Methods §3.6 (`fig:disc_render`) | Disc self-play scene, bird's-eye view |
| `gym_sim_push_t.png` | 4 Experimental Setup §4.4 (`fig:gym_render`) | Two-dimensional gym-pusht counterpart |
...
- **1.4 Contributions.** State the three from Section 0 (ASP-failure mechanism;
  ...
  isolation as the most distinctive result.
- **1.5 Thesis outline.**
...
- **2.5 The regime-contrast table.** The five-dimension comparison from Section 2.2,
...
- **3.1 Simulation environment** (existing). Now carries the rendered environment
  (`fig:env_render`, `sim_pics/isaac_enviroment_robot_setup.png`) and the
  parallel-environments render (`fig:parallel_envs`,
  `sim_pics/isaac_fullsimview_push_t_task_20_parallel_env.png`, caption states
  the 20 shown are illustrative against the 256--2{,}048 trained).
...
- **3.6 Models.** Update ASP-dPose to ABC-on; note the disc variants (F, H) and
  the Bob-penalty variant (I) as first-class subjects, not appendix items. The
  rendered self-play scene (`fig:asp_render`,
  `sim_pics/isaac_push_t_asp_red-alice_blue-bob.png`) follows the ASP loop
  diagram, and the disc scene (`fig:disc_render`,
  `sim_pics/isaac_birdview_disc_asp_alice-red_bob-blue.png`) sits with the
  coupling-isolation paragraph.
...
- **3.9 Validation protocol** (now a pointer to the Experiments chapter; the
  budget lives only in `sec:val_protocol` per rule §1.8; the three methodological
  protocol decisions stay here).
- **3.10 Training instrumentation** (existing value-error + ABC-clip logging).
...
- **4.3 Validation protocol** (existing; both validators sample; 30 scenes,
  combined gate; seed-mean and CI reporting). This section is the single source
  of truth for the evaluation budget (attempts per scene, pushes per attempt);
  no other part of the thesis may restate it — see rule §1.8. The two passes are
  described in plain terms ("abbreviated evaluation" / "full evaluation"), never
  as "fast pass" / "best-of-20".
- **4.4 Cross-environment protocol** (existing). Now carries the gym-pusht render
  (`fig:gym_render`, `sim_pics/gym_sim_push_t.png`) and the cross-simulator
  evaluation budget (20 attempts per scene).
...
- **4.6 Metrics.** Scene SR, trial SR, per-axis SR, combined SR, value error,
  ABC clip-fraction and BC-ratio, average pushes.
...
- **5.1 The task is solvable (anchor).** Single-agent PBRS reaches its success
  ...
  Establishes that ASP's failure is not task-impossibility. Carries
  `per_scene_errors_3models.png`.
...
- **5.3 RQ1: ASP fails here.** E and G scene SR with CIs, far below single-agent,
  ...
  Carries the regenerated `p3_heatmap.png` (per-scene outcomes, all six validated
  models) as the granular companion to the headline bars.
- **5.4 RQ2 dimension 1 — goal coupling (the key result).** disc F/H (position-only
  ...
  Carries the regenerated `plot3_failure_taxonomy.png` (success rectangle).
- **5.5 RQ2 dimension 2 — reward formulation.** E (outcome) vs G (time-based).
  Carries the regenerated `asp_vs_tasp_training.png` (Alice displacement and
  goal validity over training).
- **5.6 RQ2 dimension 3 — imitation.** ABC on vs off (E and G); the positive control
  (disc ABC on vs off) showing ABC helps when the task is easy.
- **5.7 RQ2 dimension 4 — goal representation.** Goal encoder on vs off (E and G).
- **5.8 RQ2 dimension 5 — compute scale.** ASP scale sweep; combined SR flat (or not)
...
- **5.9 RQ3: the mechanism.** Per-axis vs combined SR (skills learned, not
  ...
  ABC clip-saturation measurement.
...
- **6.1 Synthesis: why ASP fails here and succeeds elsewhere.** Walk the five
  dimensions, concluding which one(s) the data implicates most strongly (expected:
  coupled gate, with compute ruled out by the scale sweep).
- **6.2 Relation to prior work** (existing, updated): Ng confirmed; Plappert's ABC
  clip mechanism explains the discrete-action failure; Sukhbaatar's
  by-construction achievability does not extend beyond the training goal
  distribution; Berner's scale story tested and ruled out here.
- **6.3 Contributions revisited**, mapped to the three of Section 0.
- **6.4 Limitations.** Best-of-20 inflates scene SR; ASP differs on several dimensions
  though single-dimension isolations are provided; PBRS parameter sensitivity is a
...
- **J** Regime-contrast disaggregation and the dimensions ASP varies on at once.
- **K** Training-budget and update-count table (batch-vs-update confound).
- **L** SAC + HER configuration and status.
...
|---|---|---|---|
| Validation CSV | one CSV per run dir | `final_results_thesis/analysis/val_data/validation_results_130826/<run>/validation_results_*.csv` | per-scene: `test_index, test_name, test_type, object_type, success, pushes_used, pos_err, rot_err, area_coverage, trial_count, success_count` (one row per scene per seed; Isaac rows aggregate 3 attempts, gym rows 20) |
| TensorBoard event files | `.tfevents` | `final_results_thesis/<exp>/summary/` | training curves: `Metrics/Bob/CombinedSR, PositionSR, RotationSR, ValueError, ABC/ClipFraction, Diagnostics/GradientUpdates, Metrics/Alice/MeanDisp3D, Metrics/Alice/GoalValidityRate` |
| Collated summary | CSV + MD | `analysis/val_data/collated_summary.{csv,md}` produced by `val_collate.py` | per-model mean ± 95 % CI for scene SR, trial SR, per-test-type SR |
| TB extraction | CSV | `analysis/tb_data/` produced by `extract_tb_data.py` | `tb_final_values.csv`, `tb_seed_means.csv`, `tb_curves/*.csv` |
| Figures | PNG + PDF | `analysis/plots_table/`; PNG copies into thesis `images/` | `plot_results.py` (fig1–fig8 + headline table), `plot_presentation_figs.py`, `make_error_figure.py` |
...
> **Tag-definition ledger (verified against the training scripts,
> 2026-08-13).** `Metrics/SuccessRate` (single-agent) and
> `Metrics/Bob/CombinedSR` / `Metrics/Bob/SuccessRate` (self-play) are
> **per-push** rates: the fraction of pushes in the sliding window of the last
> rollout whose end state meets the combined gate (position gate for the disc).
> `Metrics/EpisodicSR` (single-agent only) is the per-episode rate over the
> 5-push episodes. The headline training-time success rate is the per-push tag
> for every model; the per-episode tag (single-agent, about 45.5 %) is not used
> in the headline tables. Rollout length is 15 pushes for the single agent and
> 5 + 10 for self-play; the episode budget is 5 pushes for the single agent and
> 10 for the goal-solver.
...
> Status 2026-08-13: the layout above is a template. The filled numbers now live
> in `final_results_thesis/analysis/val_data/collated_summary.md` and in
> `results.tex` Table 1; the table keys are the collated groups (PBRS 79.3,
> Curriculum 80.0, ASP-disc 7.3, TASP-disc 6.7, ASP-dPose 0.0, TASP-dPose 0.0,
> Bob-penalty 0.0 at 528 envs, with gym-PBRS at 64 envs). Training-time success
> rates are per-push combined-gate rates with 95 % CIs: PBRS 10.9 ± 1.8,
> Curriculum 12.1 ± 2.7, ASP-dPose 35.8 ± 8.8 (5 seeds, crashed seed included),
> TASP-dPose 32.6 ± 0.4, Bob-penalty 32.5 ± 0.7, ASP-disc 77.9 ± 17.2,
> TASP-disc 83.8 ± 9.4.

### 6.4 Figure inventory

Figures are generated by the scripts in `final_results_thesis/analysis/`,
following the conventions of the earlier `plot_push_primitive.py` (matplotlib,
Agg backend, ColorBrewer palette, DPI 150, PDF + PNG output to `plots_table/`,
PNG copies to the thesis `images/`). All figure-embedded source notes obey
rule §1.8: no evaluation budgets, no campaign names.
...
**Figure 2 — coupling isolation (RQ2 dimension 1, the flagship).**
...
**Figure 3 — scale sweep (RQ2 dimension 5).**
...
**Figure 4 — component ablations (RQ2 dimensions 3–4).**
...
**Figure 6 — training-time diagnostics (RQ2 dimension 3, RQ3).**
...
**Figure 7 — cross-environment (supplementary).**
...
**Presentation-figure regens (`plot_presentation_figs.py`).** Three figures
from the presentation deck, regenerated 1:1 with campaign data:
- `asp_vs_tasp_training.png` — Alice mean goal displacement (top) and goal
  validity rate (bottom) over training, ASP-dPose vs TASP-dPose, seed mean ± 1
  std from TensorBoard (`Metrics/Alice/MeanDisp3D`,
  `Metrics/Alice/GoalValidityRate`). Crashed seeds are dropped by coverage
  threshold; the exclusion is documented in `sec:train_config`.
- `p3_heatmap.png` — per-scene outcomes heatmap, 6 model rows (PBRS,
  Curriculum, ASP-dPose, TASP-dPose, Bob-penalty, gym-PBRS) × 30 T-block scenes,
  cells = successful attempts / attempts across seeds, block separators at the
  three scene groups.
- `plot3_failure_taxonomy.png` — three-panel pos × rot error scatter with the
  success rectangle, PBRS / Curriculum / ASP-dPose, one seed-mean point per
  scene.

**Per-scene error figure (`make_error_figure.py`).** `per_scene_errors_3models.png`
— 2×3 bar figure (position/orientation error rows × PBRS/Curriculum/ASP-dPose
columns), green/red by seed-majority scene success.

### 6.5 Execution order

1. Run `final_results_thesis/analysis/val_collate.py` on the done runs — get
   headline numbers into `collated_summary.csv`.
2. After all running jobs finish, re-run the collator on the full set.
3. Extract TensorBoard curves for per-axis-vs-combined, ABC clip-fraction,
   value error, and the Alice diagnostics using `extract_tb_data.py`.
4. Run `plot_results.py` (fig1–fig8), `plot_presentation_figs.py`
   (asp_vs_tasp_training, p3_heatmap, plot3_failure_taxonomy), and
   `make_error_figure.py` (per_scene_errors_3models) — all read
   `collated_summary.csv` and the TB event files and write to `plots_table/`
   plus PNG copies into the thesis `images/`.
5. Write the Results chapter, slotting each figure into the corresponding
   section (fig1→§5.3 RQ1, fig2→§5.4 RQ2 dimension 1, figs 3–4→§5.5–5.8 RQ2 dimensions 2–5,
   figs 5–6→§5.9 RQ3 mechanism, fig7→§5.10 cross-env; the presentation-figure
   regens→§5.1/§5.3–§5.5 as marked in Section 3.2).
...
items 1–2; the figure regeneration by items 3–4; the chapter drafting order
