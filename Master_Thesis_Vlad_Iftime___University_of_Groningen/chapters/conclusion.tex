...
\oldresults{old single-seed numbers and the old reward-vs-training framing;
to be rewritten around the ASP-failure regime analysis.}
...
\oldresults{conceptual arguments are kept, but the embedded gym curriculum
numbers (10.0\% to 50.0\%) are old single-seed values.}
...
\oldresults{stated against the old RQ1--RQ3 with old single-seed numbers;
to be remapped to the three contributions of the ASP-failure framing.}
...
\oldresults{states the single-seed / no-confidence-interval limitation that the
new experiment set (five seeds with confidence intervals) removes; must be updated.}
...
\oldresults{several items listed here (multi-seed confidence intervals, batch/scale
sweep, ABC-clip instrumentation, disc isolation) are already performed by the
experiment set; reconcile once its results exist.}
...
\oldresults{old priority-ordering message and old numbers (4x, 35 percentage
points); to be rewritten as the ASP-failure conclusion.}
