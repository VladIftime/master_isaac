"""
Push-ASP PBRS Model D — GoalEncoder ablation for Model C.
 
Identical to Model C except Bob's GoalEncoder is removed.  Bob's PI-encoder
sees the full per-object observation chunk (obj_state + goal_pose + dist)
directly, without the 8D compression bottleneck.  All other architecture
and training dynamics are unchanged — ASP two-phase loop, Alice unchanged,
PBRS dense reward, PPOABC + ABC buffer, historical pool.
 
Purpose: isolate whether the GoalEncoder's 8D latent compression (difference
variant, max-pooled across objects) prevents Bob from learning translation
under PBRS reward shaping and adversarial Alice curriculum.
...
Architecture:
  ...
  - Bob:   PI-encoder (full 22D per-obj chunk) + LSTM, 4D x 21-bin
           MultiCategorical, NO GoalEncoder (ablated)
...
Run locally:
  python -m asyncDualPlayPPO.train_d_pbrs_asp_noge --num_envs 16 --max_iterations 500 --exp_name push_pbrs_d_test --headless
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Train Push-Primitive Asymmetric Self-Play (PBRS Model D — GoalEncoder ablated)"
    )
    parser.add_argument("--exp_name", type=str, default="push_pbrs_d")
    ...
    print(
        f"[Config] Push-ASP PBRS-D (GoalEncoder ablated): alice_pushes={alice_pushes}, bob_pushes={bob_pushes}, "
    ...
    bob_cfg["policy"]["use_goal_encoder"] = False
    ...
    print(f"To resume:\n  python -m asyncDualPlayPPO.train_c_pbrs_asp --exp_name {args.exp_name} \\")
