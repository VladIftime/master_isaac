"""
Push-ASP PBRS Model G training script (T-block + SE(2) d_pose + time-based ASP).

Same as Model E but replaces Alice's outcome-based reward (+5 fail / -1 succeed,
...
This restores the self-regulating curriculum property from Sukhbaatar et al. 2018:
Alice is incentivized to find the simplest task (fewest pushes) that maximizes
Bob's solve time, naturally sitting at the frontier of Bob's capability.
...
Run locally:
  python -m asyncDualPlayPPO.train_push_pbrs_tasp_dpose --num_envs 16 --max_iterations 500 --exp_name push_pbrs_tasp_dpose_test --headless
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Train Push-Primitive ASP with time-based reward + d_pose (PBRS Model G)"
    )
    ...
    print(
        f"[Config] Push-ASP PBRS-G (time-based + d_pose): alice_pushes={alice_pushes}, bob_pushes={bob_pushes}, "
