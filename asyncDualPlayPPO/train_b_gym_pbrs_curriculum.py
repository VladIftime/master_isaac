"""
gym-pusht PBRS Model B — single-agent PPO + forced pos->rot curriculum (P82).

Smart-env architecture (GymPushPrimitiveEnv + AsyncVectorEnv).  Identical to
train_a_gym_pbrs_simple except the curriculum controller, which is applied to
the vectorized envs each iteration via adapter.set_curriculum(...):
  Phase 1: w_rot=0, enable_rot_sparse=False, pos_term_threshold=0.05.
  Trigger: episodic position-only SR >= 0.5 for CURRICULUM_LOOKBACK iters.
  Phase 2: pos_term_threshold=0.0, enable_rot_sparse=True, ramp w_rot 0->10
           over CURRICULUM_RAMP_ITERS.

Run:
  .master_venv/bin/python3 -m asyncDualPlayPPO.train_b_gym_pbrs_curriculum \
      --num_envs 16 --max_iterations 50 --exp_name gym_pbrs_b_smoke
"""
...
from asyncDualPlayPPO.tasks.utils.reward_pbrs import PBRS_W_POS, PBRS_W_ROT
...
def main():
    p = argparse.ArgumentParser(description="gym-pusht PBRS Model B (curriculum)")
    p.add_argument("--exp_name", type=str, default="gym_pbrs_b")
    ...
    print(f"[gym-B] num_envs={env.num_envs} device={device} obs_dim={env.obs_dim}")
    ...
    # ── Curriculum state (P82) ────────────────────────────────────────────────
    curriculum_active = False
    ramp_start = None
    RAMP_ITERS = 200
    ENTER_THRESHOLD = 0.5
    LOOKBACK = 20
    pos_sr_buf = deque(maxlen=LOOKBACK)
    w_rot = 0.0
    pos_term = 0.05
    env.set_curriculum(w_rot=0.0, pos_term_threshold=pos_term, enable_rot_sparse=False)
    ...
    while iteration < args.max_iterations:
        ...
        for _ in range(push_nsteps):
            ...
            if dmask.any():
                succ = np.asarray(info["success"], dtype=bool)
                ponly = np.asarray(info["pos_only"], dtype=bool)
                ep_succ.extend(succ[dmask].tolist())
                pos_oks.extend((succ | ponly)[dmask].astype(np.float32).tolist())
        ...
        # ── Curriculum trigger ────────────────────────────────────────────────
        pos_sr_this = float(np.mean(pos_oks)) if pos_oks else 0.0
        pos_sr_buf.append(pos_sr_this)
        if (not curriculum_active and len(pos_sr_buf) == LOOKBACK
                and all(s >= ENTER_THRESHOLD for s in pos_sr_buf)):
            curriculum_active = True
            ramp_start = iteration
            pos_term = 0.0
            print(f"[Curriculum] Phase 2 at iter {iteration} "
                  f"(posSR {min(pos_sr_buf):.2f}-{max(pos_sr_buf):.2f})", flush=True)
        w_rot = PBRS_W_ROT * min(1.0, (iteration - ramp_start) / RAMP_ITERS) if curriculum_active else 0.0
        env.set_curriculum(w_rot=w_rot, pos_term_threshold=pos_term,
        ...
        phase = "P1" if not curriculum_active else (f"P2({w_rot:.1f})" if w_rot < PBRS_W_ROT else "P3")
        ...
        print(f"[Iter {iteration:5d}|{phase}] Loss={loss_surr:.4f} Val={loss_val:.4f} "
              ...
              f"PosEpiSR={pos_sr_this:.3f} w_rot={w_rot:.1f} Ep={len(ep_succ)} Best={best_sr:.4f}",
              flush=True)
