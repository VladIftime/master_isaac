...
class GPUDemonstrationBuffer:
    def __init__(self, capacity, obs_shape, action_shape, device):
        ...
        self.old_log_probs = torch.zeros((capacity, 1), dtype=torch.float32, device=device)
    ...
    def add_batch(self, obs, actions, log_probs):
        ...
        indices = torch.arange(self.ptr, self.ptr + num_samples, device=self.device) % self.capacity
