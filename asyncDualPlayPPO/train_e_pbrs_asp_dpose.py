"""
Push-ASP PBRS Model E training script (T-block + SE(2) d_pose).

Same as Model C but replaces the dual position/rotation PBRS objectives with a
single SE(2) d_pose metric: d_pose = sqrt(dx^2 + dy^2 + L^2 * dtheta^2) where
L is the characteristic length of the object.  The observation presents
[d_pose, bearing] in place of [pos_dist, rot_dist], and a single PBRS potential
Phi(s) = exp(-k * d_pose^2) replaces the separate position and rotation
potentials.  Success is measured by d_pose < threshold (single threshold).

Action space: 4D MultiCategorical (Xs, Ys, length, theta) — unchanged from C.
...
Architecture:
  ...
  - Rewards: PBRS dense reward using single d_pose potential + sparse completion
...
Run locally:
  python -m asyncDualPlayPPO.train_e_pbrs_asp_dpose --num_envs 16 --max_iterations 500 --exp_name push_pbrs_dpose_test --headless
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Train Push-Primitive ASP with d_pose metric (PBRS Model E)"
    )
    parser.add_argument("--exp_name", type=str, default="push_pbrs_dpose")
    ...
    print(
        f"[Config] Push-ASP PBRS-E (d_pose): alice_pushes={alice_pushes}, bob_pushes={bob_pushes}, "
    ...
    # Combined-gate SR (pos<0.05m AND rot<0.2rad) — matches the validation gate,
    # so model_best is selected on the same criterion it is scored on (V2 fix).
    ...
    print(f"To resume:\n  python -m asyncDualPlayPPO.train_e_pbrs_asp_dpose --exp_name {args.exp_name} \\")
