"""
Validate a trained SAC+HER push model against test configurations.

Uses the same protocol as validate_push.py (same physics env, waypoints, IK)
but loads SAC checkpoints via SB3 instead of PPO + ActorCriticPush.

Usage:
  python -m asyncDualPlayPPO.tests.validate_push_sac \
      --chkpt runs/sac/push_primitive/.../agent_final.zip \
      --num_tests 10 --headless --csv results.csv
"""
...
def main():
    parser = argparse.ArgumentParser(description="Validate SAC+HER Push Model")
    parser.add_argument("--chkpt", type=str, required=True,
                        help="Path to SB3 SAC checkpoint (.zip)")
    ...
    parser.add_argument("--rel-act", action="store_true", dest="rel_act",
                        help="Decode actions as object-relative")
    ...
    if args.csv is None:
        args.csv = os.path.join(chkpt_run_dir, "validation_results_sac.csv")
    ...
    _goal_viz = VisualizationMarkers(
        VisualizationMarkersCfg(
            ...
            markers={"tblock": UsdFileCfg(
                usd_path=os.path.join(os.path.dirname(os.path.dirname(__file__)),
                                      "assets/blocks/t_shape.usda"),
                scale=(2.0, 2.0, 0.01),
    ...
    class _DummyVecEnv(VecEnv):
        def __init__(self, obs_space, act_space, num_envs=1):
        ...
        def step_wait(self): return np.zeros((1, 0)), np.zeros(1), np.ones(1, dtype=bool), [{}]
        ...
        def seed(self, seed=None): return None
        def get_images(self): return []

    obs_feat_dim = 22
    goal_dim = 3
    obs_space = gym_mc.spaces.Dict({
        "observation": gym_mc.spaces.Box(
            -np.inf, np.inf, shape=(obs_feat_dim,), dtype=np.float32),
        "achieved_goal": gym_mc.spaces.Box(
            -np.inf, np.inf, shape=(goal_dim,), dtype=np.float32),
        "desired_goal": gym_mc.spaces.Box(
            -np.inf, np.inf, shape=(goal_dim,), dtype=np.float32),
        "policy": gym_mc.spaces.Box(
            -np.inf, np.inf, shape=(obs_feat_dim,), dtype=np.float32),
    })
    act_space = gym_mc.spaces.Box(-1.0, 1.0, shape=(4,), dtype=np.float32)
    dummy_env = _DummyVecEnv(obs_space, act_space)

    model = SAC.load(args.chkpt, env=dummy_env, seed=42)
    model.policy.set_training_mode(False)
    print(f"[Validate] Loaded SAC model from {args.chkpt}")
    print(f"[Validate] Mode: rel_act={args.rel_act}, "
          f"rot_threshold={args.rot_threshold:.3f} rad, max_pushes={args.max_pushes}")

    # ── Build SAC dict observation from flat wrapper obs ───────────────────────
    def _build_sac_dict_obs(flat_obs: torch.Tensor) -> dict:
        obj_pos = flat_obs[0, 6:9]
        obj_euler = flat_obs[0, 9:12]
        goal_pos = flat_obs[0, 20:23]
        goal_euler = flat_obs[0, 23:26]

        rel_dx = goal_pos[0:1] - obj_pos[0:1]
        rel_dy = goal_pos[1:2] - obj_pos[1:2]

        observation = torch.cat([
            flat_obs[0, 0:3], flat_obs[0, 3:6],
            obj_pos, obj_euler,
            flat_obs[0, 12:15], flat_obs[0, 15:18],
            flat_obs[0, 18:20],
            rel_dx, rel_dy,
        ], dim=-1)

        achieved_goal = torch.cat([obj_pos[:2], obj_euler[2:3]], dim=-1)
        desired_goal = torch.cat([goal_pos[:2], goal_euler[2:3]], dim=-1)
    ...
    for test_idx in range(1, n_tests + 1):
        ...
        for trial in range(TRIAL_COUNT):
            ...
            for push_i in range(args.max_pushes):
                sac_obs = _build_sac_dict_obs(obs)
