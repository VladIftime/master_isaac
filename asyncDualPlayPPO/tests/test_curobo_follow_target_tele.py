...
def main():
    ...
    try:
        ...
        pass
    # Null out observation terms that reference the removed scene entities.
    ...
    # Custom Remapping: Add Triggers for Z-axis (LT=Down, RT=Up)
    # This helps if the Right Stick isn't working or if you prefer triggers for Z.
    def _remap_z(val, axis_idx, direction):
        gamepad._delta_pose_raw[direction, axis_idx] = gamepad.pos_sensitivity * val

    gamepad.add_callback(carb.input.GamepadInput.LEFT_TRIGGER, lambda: _remap_z(1.0, 2, 1))
    gamepad.add_callback(carb.input.GamepadInput.RIGHT_TRIGGER, lambda: _remap_z(1.0, 2, 0))
    gamepad.reset()
    ...
    for _tcp_name in ["grasp_convenient_link", "robotiq_arg2f_base_link", "wrist_3_link"]:
        try:
            tcp_ids, _ = robot.find_bodies([_tcp_name])
            tcp_id = tcp_ids[0]
            print(f"  TCP body: {_tcp_name} (id={tcp_id})")
            break
        except Exception:
            continue
    else:
        raise RuntimeError(f"No TCP body found. Available: {robot.body_names}")

    # ── Spawn red target sphere ───────────────────────────────────────────────
    ...
    # Hard clamp bounds — ball is never allowed outside these (world frame).
    _WS_X_MIN, _WS_X_MAX = -0.75,  0.75
    _WS_Y_MIN, _WS_Y_MAX =  0.20,  1.00
    _WS_Z_MIN, _WS_Z_MAX =  0.02,  1.05
    _WS_MAX_REACH = 0.70  # radial limit in XY; tighter than IK MAX_REACH

    # Visual border cuboids are drawn 3 cm outside the hard clamp so the user
    # sees a visible wall just before the ball hits the invisible hard stop.
    _VIS_MARGIN = 0.03
    _vb = _VIS_MARGIN
    ...
    # _xlate_ops reads the COMPOSED (strongest-layer) value — this picks up gizmo
    # session-layer overrides, unlike XformCommonAPI.GetXformVectors().
    _xlate_ops = [
        op for op in UsdGeom.Xformable(target_prim).GetOrderedXformOps()
        if op.GetOpType() == UsdGeom.XformOp.TypeTranslate
    ]

    # Robot base position needed for the radial clamp — available now after env.reset().
    _base_x = robot.data.root_pos_w[0, 0].item()
    _base_y = robot.data.root_pos_w[0, 1].item()
    ...
    def _enforce_bounds(_event):
        ...
        Applies both the rectangular workspace box AND a radial reach clamp so the
        ball can never be placed at a position the IK solver cannot handle."""
        if not _xlate_ops:
            return
        t = _xlate_ops[0].Get()
        if t is None:
        ...
        # Radial clamp: keep ball within reachable sphere around robot base (XY plane).
        dx, dy = x - _base_x, y - _base_y
        horiz = math.sqrt(dx * dx + dy * dy)
        if horiz > _WS_MAX_REACH:
            scale = _WS_MAX_REACH / horiz
            x, y = _base_x + dx * scale, _base_y + dy * scale
        _ball_pos[0], _ball_pos[1], _ball_pos[2] = x, y, z
        if x != t[0] or y != t[1] or z != t[2]:
            with Usd.EditContext(stage, _session_layer):
                _xlate_ops[0].Set(Gf.Vec3d(x, y, z))
    ...
    try:
        while simulation_app.is_running():
            ...
            # Read gamepad input and add it to the target's USD translation.
            gamepad_cmd = gamepad.advance()
            delta_pos = gamepad_cmd[:3].cpu().numpy()
            if delta_pos.any():
                carb.log_info(f"Gamepad Command [XYZ]: {delta_pos.round(4)}")
                t = _xlate_ops[0].Get()
                if t is not None:
                    new_x = float(t[0] + delta_pos[0])
                    new_y = float(t[1] + delta_pos[1])
                    new_z = float(t[2] + delta_pos[2])
                    with Usd.EditContext(stage, _session_layer):
                        _xlate_ops[0].Set(Gf.Vec3d(new_x, new_y, new_z))
                    # Update local _ball_pos immediately so the current IK step sees the new target.
                    # (The _enforce_bounds callback will clamp it next frame if it overshoots).
                    _ball_pos[0], _ball_pos[1], _ball_pos[2] = new_x, new_y, new_z
