...
namespace OgnSyncGateAttributes
{
namespace inputs
{
...
using syncValue_t = const uint64_t&;
ogn::AttributeInitializer<const uint64_t, ogn::kOgnInput> syncValue("inputs:syncValue", "uint64", kExtendedAttributeType_Regular, 0);
}
namespace outputs
{
...
using syncValue_t = uint64_t&;
ogn::AttributeInitializer<uint64_t, ogn::kOgnOutput> syncValue("outputs:syncValue", "uint64", kExtendedAttributeType_Regular);
}
namespace state
{
}
}
using namespace OgnSyncGateAttributes;
namespace IOgnSyncGate
{
...
class OgnSyncGateDatabase : public omni::graph::core::ogn::OmniGraphDatabase
{
public:
    template <typename StateInformation>
    CARB_DEPRECATED("sInternalState is deprecated. Use sSharedState or sPerInstanceState instead")
    static StateInformation& sInternalState(const NodeObj& nodeObj, InstanceIndex index = {kAuthoringGraphIndex}) {
        return sm_stateManagerOgnSyncGate.getState<StateInformation>(nodeObj.nodeHandle, index);
    }
    template <typename StateInformation>
    static StateInformation& sSharedState(const NodeObj& nodeObj) {
        return sm_stateManagerOgnSyncGate.getState<StateInformation>(nodeObj.nodeHandle, kAuthoringGraphIndex);
    }
    template <typename StateInformation>
    ...
    static StateInformation& sPerInstanceState(const NodeObj& nodeObj, GraphInstanceID instanceId) {
        return sm_stateManagerOgnSyncGate.getState<StateInformation>(nodeObj.nodeHandle, instanceId);
    }
    template <typename StateInformation>
    ...
    static ogn::StateManager sm_stateManagerOgnSyncGate;
    static std::tuple<int, int, int>sm_generatorVersionOgnSyncGate;
    static std::tuple<int, int, int>sm_targetVersionOgnSyncGate;
    ...
    struct inputsT {
        ...
        ogn::SimpleInput<const uint64_t,ogn::kCpu> syncValue;
    } inputs;
    ...
    struct outputsT {
        ...
        ogn::SimpleOutput<uint64_t,ogn::kCpu> syncValue;
    } outputs;

    //Only use this constructor for temporary stack-allocated object:
    OgnSyncGateDatabase(NodeObj const& nodeObjParam)
    : OmniGraphDatabase()
    ...
    OgnSyncGateDatabase(GraphContextObj const&, NodeObj const& nodeObjParam)
    : OgnSyncGateDatabase(nodeObjParam)
    {}

    //Main constructor
    OgnSyncGateDatabase(GraphContextObj const* contextObjParam, NodeObj const* nodeObjParam, size_t handleCount)
    : OmniGraphDatabase()
    , inputs{m_offset.index}
    , outputs{m_offset.index}
    {
        ...
        NodeObj const& nodeObj = abi_node();
        {
            auto inputDataHandles0 = getAttributesR<
                ...
                >(contextObj, nodeObj.nodeContextHandle, std::make_tuple(
                    inputs::execIn.m_token, inputs::syncValue.m_token
                )
            , kAccordingToContextIndex);
            auto outputDataHandles0 = getAttributesW<
                AttributeDataHandle, AttributeDataHandle
                >(contextObj, nodeObj.nodeContextHandle, std::make_tuple(
                    outputs::execOut.m_token, outputs::syncValue.m_token
                )
            , kAccordingToContextIndex);
            inputs.execIn.setContext(contextObj);
            inputs.execIn.setHandle(std::get<0>(inputDataHandles0));
            inputs.syncValue.setContext(contextObj);
            inputs.syncValue.setHandle(std::get<1>(inputDataHandles0));
            outputs.execOut.setContext(contextObj);
            outputs.execOut.setHandle(std::get<0>(outputDataHandles0));
            outputs.syncValue.setContext(contextObj);
            outputs.syncValue.setHandle(std::get<1>(outputDataHandles0));
        }
...
public:
    static void initializeType(const NodeTypeObj& nodeTypeObj)
    {
        ...
        if( ! iTokenPtr ) {
            CARB_LOG_ERROR("IToken not found when initializing omni.graph.action.SyncGate");
            return;
        }
        auto& iToken{ *iTokenPtr };

        inputs::execIn.initialize(iToken, *iNodeType, nodeTypeObj);
        inputs::syncValue.initialize(iToken, *iNodeType, nodeTypeObj);

        outputs::execOut.initialize(iToken, *iNodeType, nodeTypeObj);
        outputs::syncValue.initialize(iToken, *iNodeType, nodeTypeObj);

        iNodeType->setMetadata(nodeTypeObj, kOgnMetadataExtension, "omni.graph.action_nodes");
        iNodeType->setMetadata(nodeTypeObj, kOgnMetadataUiName, "Sync Gate");
    ...
    static void initialize(const GraphContextObj&, const NodeObj& nodeObj)
    {
        ...
        attr = iNode->getAttributeByToken(nodeObj, inputs::syncValue.token());
        attr.iAttribute->setMetadata(attr, kOgnMetadataDescription, "Value against which each of the input signals are checked for synchronization.");
        attr.iAttribute->setMetadata(attr, kOgnMetadataUiName, "Sync");
        ...
        attr = iNode->getAttributeByToken(nodeObj, outputs::syncValue.token());
        attr.iAttribute->setMetadata(attr, kOgnMetadataDescription, "Current synchronization value, independent of whether the output activation signal has been triggered.");
        attr.iAttribute->setMetadata(attr, kOgnMetadataUiName, "Sync");
    }
    ...
    {
        sm_stateManagerOgnSyncGate.removeState(nodeObj.nodeHandle, instanceID);
    }
    bool validate() const {
        return validateNode()
            && inputs.execIn.isValid()
            && inputs.syncValue.isValid()
            && outputs.execOut.isValid()
            && outputs.syncValue.isValid()
        ;
    }
    void preCompute() {
        if(m_canCachePointers == false) {
            inputs.execIn.invalidateCachedPointer();
            inputs.syncValue.invalidateCachedPointer();
            outputs.execOut.invalidateCachedPointer();
            outputs.syncValue.invalidateCachedPointer();
            return;
        }
        for(NameToken const& attrib : m_mappedAttributes) {
            ...
            if(attrib == inputs::syncValue.m_token) {
                inputs.syncValue.invalidateCachedPointer();
                continue;
            }
            ...
            if(attrib == outputs::syncValue.m_token) {
                outputs.syncValue.invalidateCachedPointer();
                continue;
            }
            bool found = false;
    ...
    bool canVectorize() const {
        if( !inputs.execIn.canVectorize()
            || !inputs.syncValue.canVectorize()
            || !outputs.execOut.canVectorize()
            || !outputs.syncValue.canVectorize()
        ) return false;
    ...
    void onDataLocationChanged(AttributeObj const& attr) {
        ...
        if(token == inputs::syncValue.m_token) {
            ConstAttributeDataHandle hdl = attr.iAttribute->getConstAttributeDataHandle(attr, m_offset);
            inputs.syncValue.setHandle(hdl);
            return;
        }
        ...
        if(token == outputs::syncValue.m_token) {
            AttributeDataHandle hdl = attr.iAttribute->getAttributeDataHandle(attr, m_offset);
            outputs.syncValue.setHandle(hdl);
            return;
        }
        for (auto& __a : m_dynamicInputs) {
            if (__a().name() == token) {
...
ogn::StateManager OgnSyncGateDatabase::sm_stateManagerOgnSyncGate;
std::tuple<int, int, int> OgnSyncGateDatabase::sm_generatorVersionOgnSyncGate{std::make_tuple(1,79,2)};
std::tuple<int, int, int> OgnSyncGateDatabase::sm_targetVersionOgnSyncGate{std::make_tuple(2,184,5)};
}
using namespace IOgnSyncGate;
#define REGISTER_OGN_NODE() \
namespace { \
    ogn::NodeTypeBootstrapImpl<OgnSyncGate, OgnSyncGateDatabase> s_registration("omni.graph.action.SyncGate", 2, "omni.graph.action_nodes"); \
}
