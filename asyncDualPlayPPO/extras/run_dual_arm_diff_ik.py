...
parser = argparse.ArgumentParser(
    description="Dual-arm differential IK controller demonstration."
)
parser.add_argument(
...
parser.add_argument(
    "--sync_mode",
    action="store_true",
    help="If set, both arms move to the same goal. Otherwise, independent goals.",
)
parser.add_argument(
    "--goal", type=int, default=0, help="Goal index to move to (0, 1, 2, etc.)"
)
...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene):
    ...
    diff_ik_left = DifferentialIKController(
        diff_ik_cfg, num_envs=scene.num_envs, device=sim.device
    )
    diff_ik_right = DifferentialIKController(
        diff_ik_cfg, num_envs=scene.num_envs, device=sim.device
    )
    ...
    ik_commands_left = torch.zeros(
        scene.num_envs, diff_ik_left.action_dim, device=robot.device
    )
    ik_commands_right = torch.zeros(
        scene.num_envs, diff_ik_right.action_dim, device=robot.device
    )
    ...
    def set_goal(goal_idx):
        ...
        print(
            f"[GOAL {goal_idx}/{num_goals-1}] Left: {left_goals[goal_idx][:3].tolist()}, Right: {right_goals[goal_idx][:3].tolist()}"
        )
    ...
    while simulation_app.is_running():
        ...
        jacobian_left = robot.root_physx_view.get_jacobians()[
            :, ee_jacobi_idx_left, :, left_arm_cfg.joint_ids
        ]
        ...
        joint_pos_des_left = diff_ik_left.compute(
            ee_pos_b_left, ee_quat_b_left, jacobian_left, joint_pos_left
        )

        # ===== RIGHT ARM IK =====
        jacobian_right = robot.root_physx_view.get_jacobians()[
            :, ee_jacobi_idx_right, :, right_arm_cfg.joint_ids
        ]
        ...
        joint_pos_des_right = diff_ik_right.compute(
        ...
        goal_marker_right.visualize(
            ik_commands_right[:, 0:3] + scene.env_origins, ik_commands_right[:, 3:7]
        )
