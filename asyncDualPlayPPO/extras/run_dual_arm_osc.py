...
parser = argparse.ArgumentParser(description="Dual-arm operational space controller demonstration.")
parser.add_argument("--num_envs", type=int, default=4, help="Number of environments to spawn.")
parser.add_argument("--goal", type=int, default=0, help="Goal index to start with (0, 1, 2, etc.)")
...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene):
    ...
    osc_left = OperationalSpaceController(osc_cfg, num_envs=scene.num_envs, device=sim.device)
    osc_right = OperationalSpaceController(osc_cfg, num_envs=scene.num_envs, device=sim.device)
    ...
    ee_target_set_right = torch.cat([right_goals, kp_set_task[:len(right_goals)]], dim=-1)
    ...
    while simulation_app.is_running():
        ...
        if count % 500 == 0:
            ...
            (
                ...
                _,
                ee_pose_w_right,
            ) = update_states_right(sim, scene, robot, ee_jacobi_idx_right, right_arm_cfg)
            
            # Update target pose
            command_left, ee_target_pose_b_left, ee_target_pose_w_left, current_goal_idx = update_target(
                sim, scene, osc_left, root_pose_w, ee_target_set_left, current_goal_idx
            )
            command_right, ee_target_pose_b_right, ee_target_pose_w_right, _ = update_target(
                sim, scene, osc_right, root_pose_w, ee_target_set_right, current_goal_idx
            )
            
            # Set OSC commands
            osc_left.reset()
            osc_right.reset()
            command_left, task_frame_pose_b_left = convert_to_task_frame(osc_left, command_left, ee_target_pose_b_left)
            command_right, task_frame_pose_b_right = convert_to_task_frame(osc_right, command_right, ee_target_pose_b_right)
            osc_left.set_command(command=command_left, current_ee_pose_b=ee_pose_b_left, current_task_frame_pose_b=task_frame_pose_b_left)
            osc_right.set_command(command=command_right, current_ee_pose_b=ee_pose_b_right, current_task_frame_pose_b=task_frame_pose_b_right)
            
            print(f"[GOAL {current_goal_idx}] Left: {left_goals[current_goal_idx][:3].tolist()}, Right: {right_goals[current_goal_idx][:3].tolist()}")
        else:
            # Get updated states
            (
                jacobian_b_left,
            ...
            # Apply actions
            robot.set_joint_effort_target(joint_efforts_left, joint_ids=left_arm_cfg.joint_ids)
            robot.set_joint_effort_target(joint_efforts_right, joint_ids=right_arm_cfg.joint_ids)
            robot.write_data_to_sim()
        
        # Update marker positions
        ee_marker_left.visualize(ee_pose_w_left[:, 0:3], ee_pose_w_left[:, 3:7])
        goal_marker_left.visualize(ee_target_pose_w_left[:, 0:3], ee_target_pose_w_left[:, 3:7])
        
        ee_marker_right.visualize(ee_pose_w_right[:, 0:3], ee_pose_w_right[:, 3:7])
        goal_marker_right.visualize(ee_target_pose_w_right[:, 0:3], ee_target_pose_w_right[:, 3:7])
...
def update_states_left(
    ...
    jacobian_w = robot.root_physx_view.get_jacobians()[:, ee_jacobi_idx, :, arm_cfg.joint_ids]
    mass_matrix = robot.root_physx_view.get_generalized_mass_matrices()[:, arm_cfg.joint_ids, :][:, :, arm_cfg.joint_ids]
    gravity = robot.root_physx_view.get_gravity_compensation_forces()[:, arm_cfg.joint_ids]
