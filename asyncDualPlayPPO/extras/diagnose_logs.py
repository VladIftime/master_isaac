...
LOG_DIR = Path(sys.argv[1]) if len(sys.argv) > 1 else _HERE.parent / "logs" / "long_trainining_080426"
OUT_DIR = LOG_DIR
...
BOB_UPDATE_RE = re.compile(
    r"\[Bob Update\s+(\d+)\] Loss:\s*([-\d.]+).*SR:\s*([\d.]+)"
)
...
def parse_file(path: Path) -> dict:
    ...
    d = dict(
        path=path,
        num_envs=None, nsteps=None,
        alice_timesteps=None, bob_timesteps=None,
        resume_iter=None, chain_next=None,

        # event counters
        earlyend_alice=[], earlyend_bob=[],          # list of (step, max_step, reason)
        ...
        alice_updates=0, bob_updates=0, bob_skipped=0,
        alice_update_losses=[], bob_update_srs=[],

        # SLURM metadata
        slurm_state=None, slurm_walltime=None,
        slurm_maxmem=None, slurm_gpuutil=None, slurm_gpumem=None,
        oom=False,
    )
...
def report(jobs: list[dict], out_dir: Path):
    # --- aggregate ---
    agg = dict(
        earlyend_alice=[], earlyend_bob=[],
        term_batches=Counter(),
        alice_reward_types=Counter(),
        alice_xy_displacements=[],
        alice_to_bob=0, bob_to_alice_success=0, bob_to_alice_failure=0,
        bob_success_steps=[],
        bob_success_target_dist=[], bob_success_cube_dist=[],
        alice_updates=0, bob_updates=0, bob_skipped=0,
        alice_update_losses=[], bob_update_srs=[],
        oom_count=0, total_walltime_s=0,
        gpu_utils=[],
    )
    ...
    lines.append("")
    lines.append(f"  {'File':<45} {'State':<22} {'Walltime':<12} {'MaxRAM':<10} {'GPU%'}")
    ...
    lines.append("")
    lines.append(f"  OOM jobs: {agg['oom_count']}/{len(jobs)} ({oom_pct:.0f}%)   "
    ...
    lines.append(f"  Alice early-ends: {len(alice_steps):,}  "
    ...
    lines.append("")
    lines.append(f"  Bob early-ends: {len(bob_steps):,}  "
    ...
    if agg["alice_xy_displacements"]:
        ...
        lines.append(f"    min={xyd[0]:.3f}m  p25={p25:.3f}m  median={p50:.3f}m  "
    ...
    if agg["bob_success_steps"]:
        ...
        lines.append(f"  Steps to success: min={st[0]} median={st[len(st)//2]} max={st[-1]}")
        td = agg["bob_success_target_dist"]
        cd = agg["bob_success_cube_dist"]
        lines.append(f"  Target dist at success: "
                     f"avg={sum(td)/len(td):.4f}m  max={max(td):.4f}m")
        lines.append(f"  Cube dist at success:   "
                     f"avg={sum(cd)/len(cd):.4f}m  max={max(cd):.4f}m")

    # ── Per-job summary ──────────────────────────────────────────────────────
    section("PER-JOB SUMMARY")
    lines.append(f"  {'File':<45} {'EarlyA':>7} {'EarlyB':>7} {'A→B':>6} "
...
def plot_all(agg: dict, jobs: list[dict], out_dir: Path):
    ...
    colors = ["#e74c3c", "#3498db", "#2ecc71", "#3498db", "#2ecc71", "#9b59b6", "#9b59b6"]
    ...
    if alice_steps or bob_steps:
        ...
        if alice_steps:
            ...
            axes[0].hist(alice_steps, bins=20, range=(0, alice_max),
        ...
        if bob_steps:
            ...
            axes[1].hist(bob_steps, bins=20, range=(0, bob_max),
    ...
    for j in sorted(jobs, key=lambda x: x["path"].name):
        if j["slurm_walltime"]:
            ...
            try:
                ...
                mins = 0
            wt_labels.append(j["path"].name.replace("slurm-", "").replace("-high_1647.out", ""))
            wt_vals.append(mins)
    if wt_vals:
        fig, ax = plt.subplots(figsize=(14, 4))
        colors_bar = ["#e74c3c" if j.get("oom") else "#2ecc71"
                      for j in sorted(jobs, key=lambda x: x["path"].name)
                      if j["slurm_walltime"]]
    ...
    if agg["alice_xy_displacements"]:
        ...
        ax.hist(agg["alice_xy_displacements"], bins=40, color="#2ecc71", edgecolor="white")
