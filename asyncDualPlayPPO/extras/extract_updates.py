...
def main():
    parser = argparse.ArgumentParser(description="Extract and plot metrics from async dual play PPO logs.")
    parser.add_argument("--file", type=str, default="slurm-28114045-med.out", help="Log file name in logs/ dir")
    ...
    alice_pattern = re.compile(r"\[Alice Update (\d+)\] Loss: ([\-\.\d]+) \| Val: ([\-\.\d]+) \| Rew: ([\-\.\d]+)")
    bob_pattern = re.compile(r"\[Bob Update (\d+)\] Loss: ([\-\.\d]+) \| Val: ([\-\.\d]+) \| Rew: ([\-\.\d]+) \| ABC: ([\-\.\d]+) \| SR: ([\-\.\d]+)")
    ...
    axs[2, 1].plot(iterations, alice_valid_goal_rate, label='Valid Goal Rate', color='orange')
