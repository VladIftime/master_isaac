"""
Push-ASP PBRS Model H training script (Disc + position-only d_pose + time-based ASP).

Same as Model F but replaces Alice's outcome-based reward (+5 fail / -1 succeed,
Plappert 2021) with Sukhbaatar's time-based self-play reward:
  R_A = gamma_sp * max(0, t_B - t_A)
where t_A = Alice's push count, t_B = Bob's push count, gamma_sp = 0.5.

Uses rotationally-symmetric disc (10cm diameter, 6cm tall). char_length=0 makes
d_pose collapse to pure 2D position distance. Tests whether Sukhbaatar's
self-regulation prevents the toxic curriculum observed in Model F.

Shallow goal penalty removed (time-based reward makes it redundant).
Bob reward unchanged (PBRS d_pose). ABC disabled.

Run locally:
  python -m asyncDualPlayPPO.train_h_tasp_disc --num_envs 16 --max_iterations 500 --exp_name push_pbrs_tasp_disc_test --headless
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Train Push-Primitive ASP with time-based reward + disc (PBRS Model H)"
    )
    parser.add_argument("--exp_name", type=str, default="push_pbrs_tasp_disc")
    ...
    print(
        f"[Config] Push-ASP PBRS-H (time-based + disc + d_pose): alice_pushes={alice_pushes}, bob_pushes={bob_pushes}, "
    ...
    print(f"To resume:\n  python -m asyncDualPlayPPO.train_f_pbrs_asp_disc --exp_name {args.exp_name} \\")
