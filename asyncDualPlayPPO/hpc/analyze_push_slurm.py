#!/usr/bin/env python3
"""
Analyze SLURM output files from train_push.py HPC runs.

Parses the [Iter N] and [Episode] lines from SLURM *.out files,
writes CSVs, and plots training metrics.

Usage:
  python analyze_push_slurm.py --slurm-out slurm-12345-push.out
  python analyze_push_slurm.py --slurm-out slurm-*.out --out-dir analysis/
"""
...
def parse_slurm(path: Path) -> dict:
    """Parse one SLURM output file. Returns dict of parsed records."""
...
def merge_paths(paths: list[Path]) -> dict:
    """Merge multiple SLURM files; deduplicate iters by number."""
    all_iters: list[dict] = []
...
def plot_metrics(data: dict, out_dir: Path):
    ...
    fig.suptitle("Push-PPO Training — HPC", fontsize=14, fontweight="bold")
    ...
    # --- Success rate over time (episodes) ---
    if episodes:
        fig2, ax2 = plt.subplots(figsize=(12, 4))
        # Rolling success rate (window of 50 episodes)
        window = 50
        sr_rolling = []
...
def main():
    parser = argparse.ArgumentParser(description="Analyze push-PPO SLURM output.")
    parser.add_argument(
        "--slurm-out", type=str, nargs="+", required=True,
        help="One or more SLURM *.out files to parse.",
    )
    parser.add_argument(
        "--out-dir", type=str, default=None,
        help="Output directory (defaults to directory of first slurm file).",
    )
    args = parser.parse_args()

    paths = [Path(p) for p in args.slurm_out]
    out_dir = Path(args.out_dir) if args.out_dir else paths[0].parent
    out_dir.mkdir(parents=True, exist_ok=True)

    print(f"[INFO] Parsing {len(paths)} SLURM output file(s)...")
    data = merge_paths(paths)
    print(f"[INFO] Iter records: {len(data['iters'])},  Episodes: {len(data['episodes'])}")
