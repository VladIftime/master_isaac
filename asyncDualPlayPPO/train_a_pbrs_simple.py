"""
Push-PPO PBRS Model A training script.

Single-agent PPO with push primitive macro-actions and Potential-Based Reward
Shaping (PBRS).  Dense PBRS reward signal replaces the env's built-in reward.
No curriculum — difficulty is fixed throughout training.
...
Architecture:
  ...
  waypoint → physics step → PBRS dense reward after push completes → PPO update.

Run locally:
  python -m asyncDualPlayPPO.train_a_pbrs_simple --num_envs 16 --max_iterations 500 --exp_name push_pbrs_a
"""
...
def main():
    parser = argparse.ArgumentParser(description="Push-PPO PBRS Model A Training")
    parser.add_argument("--exp_name", type=str, default="push_pbrs_a")
    ...
    from asyncDualPlayPPO.tasks.utils.reward_pbrs import (
        ...
        compute_pbrs_reward, check_done_pbrs,
    ...
    print(f"Environment ready (rel_obs=True, obs_dim={env.obs_dim}D).")
    ...
    print(f"\n{'='*80}\nPUSH-PPO PBRS MODEL A: {args.exp_name}\nLOG DIR: {run_dir}\n{'='*80}\n")
    ...
    while iteration < args.max_iterations:
        ...
        _obj_pos_pre = obs[:, env.robot_dim:env.robot_dim + 3]
        ...
        for push_step in range(push_nsteps):
            ...
            _obj_pos_now = obs[:, env.robot_dim:env.robot_dim + 3]
            _obj_euler_now = obs[:, env.robot_dim + 3:env.robot_dim + 6]
            _goal_pos_now = obs[:, env.robot_dim + env.obj_state_dim:env.robot_dim + env.obj_state_dim + 3]
            _goal_euler_now = obs[:, env.robot_dim + env.obj_state_dim + 3:env.robot_dim + env.obj_state_dim + 6]
            ...
            rot_sr_buf.extend((env._last_rot_err < 0.01).float().cpu().tolist())
            ...
            _pr(
                ...
                f"len={float(length.mean().item()):.3f} θ={math.degrees(float(theta.mean().item())):.0f}°"
            )
            ...
            if done.any():
                ...
                gave_completion[done_ids] = False
                gave_rot_bonus[done_ids] = False
