...
class RolloutStorage:
    ...
    def __init__(self, num_envs, num_transitions_per_env, obs_shape, states_shape, actions_shape, device='cpu', sampler='sequential'):
        ...
        self.masks = torch.ones(num_transitions_per_env, num_envs, 1, device=self.device).byte()
    ...
    def add_transitions(self, observations, states, actions, rewards, dones, values, actions_log_prob, mu, sigma, masks=None):
        ...
        if masks is not None:
            self.masks[self.step].copy_(masks.view(-1, 1))
        else:
            self.masks[self.step].fill_(1)
    ...
    def compute_returns(self, last_values, gamma, lam):
        ...
        for step in reversed(range(self.num_transitions_per_env)):
            ...
            # FIX: Use masks instead of dones for proper bootstrapping during phase transitions
            # If Bob transitions to Alice, mask=0, so we don't bootstrap from next (Alice) state
