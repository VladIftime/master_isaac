...
class PPOBCO(PPO):
    def __init__(
        ...
        # The IDM operates on the shared environment state (Alice's obs) so that:
        # - Training uses the Alice-space portion of Bob's rollout observations.
        # - Inference receives Alice's observations directly from train.py.
        # idm_obs_dim defaults to the full observation space if not specified.
        self.idm_obs_dim = idm_obs_dim if idm_obs_dim is not None else vec_env.observation_space.shape[0]
    ...
    def update(self):
        ...
        # Slice to idm_obs_dim: Bob's rollout contains goal info in the later dims,
        # but the IDM only sees the shared environment state (same space as Alice's obs).
        obs_t      = obs_rollout[:-1].view(-1, self.observation_space.shape[0])[:, :self.idm_obs_dim]
        obs_tplus1 = obs_rollout[1:].view(-1, self.observation_space.shape[0])[:, :self.idm_obs_dim]
